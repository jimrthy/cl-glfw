...
(defpackage #:cl-glfw-opengl
            ...
            (:nicknames #:gl #:opengl)
            (:shadowing-import-from #:cl-glfw-types #:boolean #:byte
             #:float #:char #:string #:pointer)
            (:export #:enum #:boolean #:bitfield #:byte #:short #:int
             #:sizei #:ubyte #:ushort #:uint #:float #:clampf
             #:double #:clampd #:void #:uint64 #:int64 #:intptr
             #:sizeiptr #:handle #:char #:string #:half
             #:synchronizing-program #:synchronizing-shader
             #:with-map-buffer #:with-push-name
             #:with-bind-buffer-arb #:with-map-buffer-arb
             #:check-compiled-shader-arb #:make-shader-arb
             #:clear-synchronizing-shaders #:with-projection-matrix
             #:synchronizing-shader-arb #:make-shader
             #:make-program-arb #:make-program #:with-push-matrix
             #:with-begin #:synchronizing-program-arb
             #:with-use-program #:shader-source-from-stream
             #:fallback-synchronizing-program-arb
             #:with-push-client-attrib #:with-use-program-arb
             #:with-bind-buffer #:with-setup-projection
             #:shader-source-from-stream-arb #:with-begin-query
             #:*fallback-synchronizing-program*
             #:*fallback-synchronizing-program-arb*
             #:check-compiled-shader #:fallback-synchronizing-program
             #:check-linked-program #:with-new-list
             #:with-push-attrib #:check-linked-program-arb
             #:+current-bit+ #:+point-bit+ #:+line-bit+
             #:+polygon-bit+ #:+polygon-stipple-bit+
             #:+pixel-mode-bit+ #:+lighting-bit+ #:+fog-bit+
             #:+depth-buffer-bit+ #:+accum-buffer-bit+
             #:+stencil-buffer-bit+ #:+viewport-bit+
             #:+transform-bit+ #:+enable-bit+ #:+color-buffer-bit+
             #:+hint-bit+ #:+eval-bit+ #:+list-bit+ #:+texture-bit+
             #:+scissor-bit+ #:+all-attrib-bits+ #:+multisample-bit+
             #:+coverage-buffer-bit-nv+ #:+client-pixel-store-bit+
             #:+client-vertex-array-bit+ #:+client-all-attrib-bits+
             #:+false+ #:+true+ #:+points+ #:+lines+ #:+line-loop+
             #:+line-strip+ #:+triangles+ #:+triangle-strip+
             #:+triangle-fan+ #:+quads+ #:+quad-strip+ #:+polygon+
             #:+accum+ #:+load+ #:+return+ #:+mult+ #:+add+ #:+never+
             #:+less+ #:+equal+ #:+lequal+ #:+greater+ #:+notequal+
             #:+gequal+ #:+always+ #:+zero+ #:+one+ #:+src-color+
             #:+one-minus-src-color+ #:+src-alpha+
             #:+one-minus-src-alpha+ #:+dst-alpha+
             #:+one-minus-dst-alpha+ #:+constant-color-ext+
             #:+one-minus-constant-color-ext+ #:+constant-alpha-ext+
             #:+one-minus-constant-alpha-ext+ #:+dst-color+
             #:+one-minus-dst-color+ #:+src-alpha-saturate+
             ...
             #:+emission+ #:+ambient-and-diffuse+ #:+byte+
             ...
             #:+proxy-texture-color-table-sgi+ #:+reduce-ext+
             #:+convolution-border-mode-ext+
             #:+convolution-filter-scale-ext+
             #:+convolution-filter-bias-ext+ #:+convolution-1d-ext+
             #:+convolution-2d-ext+ #:+none+ #:+front-left+
             #:+front-right+ #:+back-left+ #:+back-right+ #:+left+
             #:+right+ #:+aux0+ #:+aux1+ #:+aux2+ #:+aux3+ #:+fog+
             #:+lighting+ #:+texture-1d+ #:+texture-2d+
             #:+line-stipple+ #:+polygon-stipple+ #:+cull-face+
             #:+alpha-test+ #:+blend+ #:+index-logic-op+
             #:+color-logic-op+ #:+dither+ #:+stencil-test+
             #:+depth-test+ #:+clip-plane0+ #:+clip-plane1+
             #:+clip-plane2+ #:+clip-plane3+ #:+clip-plane4+
             #:+clip-plane5+ #:+light0+ #:+light1+ #:+light2+
             #:+light3+ #:+light4+ #:+light5+ #:+light6+ #:+light7+
             #:+texture-gen-s+ #:+texture-gen-t+ #:+texture-gen-r+
             ...
             #:+map2-texture-coord-4+ #:+point-smooth+
             #:+line-smooth+ #:+polygon-smooth+ #:+scissor-test+
             #:+color-material+ #:+normalize+ #:+auto-normal+
             #:+polygon-offset-point+ #:+polygon-offset-line+
             #:+polygon-offset-fill+ #:+vertex-array+
             #:+normal-array+ #:+color-array+ #:+index-array+
             #:+texture-coord-array+ #:+edge-flag-array+
             #:+separable-2d-ext+ #:+histogram-ext+ #:+minmax-ext+
             #:+rescale-normal-ext+ #:+shared-texture-palette-ext+
             #:+texture-3d-ext+ #:+multisample-sgis+
             ...
             #:+framezoom-sgix+ #:+interlace-sgix+
             #:+ir-instrument1-sgix+ #:+pixel-tex-gen-sgix+
             #:+pixel-texture-sgis+ #:+reference-plane-sgix+
             #:+sprite-sgix+ #:+no-error+ #:+invalid-enum+
             #:+invalid-value+ #:+invalid-operation+
             #:+stack-overflow+ #:+stack-underflow+ #:+out-of-memory+
             #:+table-too-large-ext+ #:+texture-too-large-ext+ #:+2d+
             #:+3d+ #:+3d-color+ #:+3d-color-texture+
             #:+4d-color-texture+ #:+pass-through-token+
             #:+point-token+ #:+line-token+ #:+polygon-token+
             #:+bitmap-token+ #:+draw-pixel-token+
             #:+copy-pixel-token+ #:+line-reset-token+
             #:+texture-deformation-bit-sgix+
             #:+geometry-deformation-bit-sgix+
             #:+geometry-deformation-sgix+
             #:+texture-deformation-sgix+ #:+linear+ #:+exp+ #:+exp2+
             #:+fog-func-sgis+ #:+fog-color+ #:+fog-density+
             #:+fog-end+ #:+fog-index+ #:+fog-mode+ #:+fog-start+
             ...
             #:+max-convolution-height-ext+ #:+histogram-width-ext+
             ...
             #:+coeff+ #:+order+ #:+domain+ #:+minmax-format-ext+
             #:+minmax-sink-ext+ #:+pixel-map-i-to-i+
             #:+pixel-map-s-to-s+ #:+pixel-map-i-to-r+
             #:+pixel-map-i-to-g+ #:+pixel-map-i-to-b+
             #:+pixel-map-i-to-a+ #:+pixel-map-r-to-r+
             #:+pixel-map-g-to-g+ #:+pixel-map-b-to-b+
             ...
             #:+instrument-buffer-pointer-sgix+ #:+current-color+
             #:+current-index+ #:+current-normal+
             ...
             #:+current-raster-distance+ #:+point-size+
             #:+point-size-range+ #:+point-size-granularity+
             #:+line-width+ #:+line-width-range+
             ...
             #:+aliased-line-width-range+ #:+list-mode+
             #:+max-list-nesting+ #:+list-base+ #:+list-index+
             #:+polygon-mode+ #:+edge-flag+ #:+cull-face-mode+
             #:+front-face+ #:+light-model-local-viewer+
             #:+light-model-two-side+ #:+light-model-ambient+
             #:+shade-model+ #:+color-material-face+
             #:+color-material-parameter+ #:+depth-range+
             #:+depth-writemask+ #:+depth-clear-value+ #:+depth-func+
             #:+accum-clear-value+ #:+stencil-clear-value+
             #:+stencil-func+ #:+stencil-value-mask+ #:+stencil-fail+
             #:+stencil-pass-depth-fail+ #:+stencil-pass-depth-pass+
             #:+stencil-ref+ #:+stencil-writemask+ #:+matrix-mode+
             #:+viewport+ #:+modelview-stack-depth+
             ...
             #:+alpha-test-ref+ #:+blend-dst+ #:+blend-src+
             #:+logic-op-mode+ #:+aux-buffers+ #:+draw-buffer+
             #:+read-buffer+ #:+scissor-box+ #:+index-clear-value+
             #:+index-writemask+ #:+color-clear-value+
             #:+color-writemask+ #:+index-mode+ #:+rgba-mode+
             #:+doublebuffer+ #:+stereo+ #:+render-mode+
             #:+perspective-correction-hint+ #:+point-smooth-hint+
             #:+line-smooth-hint+ #:+polygon-smooth-hint+
             #:+fog-hint+ #:+pixel-map-i-to-i-size+
             #:+pixel-map-s-to-s-size+ #:+pixel-map-i-to-r-size+
             #:+pixel-map-i-to-g-size+ #:+pixel-map-i-to-b-size+
             #:+pixel-map-i-to-a-size+ #:+pixel-map-r-to-r-size+
             #:+pixel-map-g-to-g-size+ #:+pixel-map-b-to-b-size+
             #:+pixel-map-a-to-a-size+ #:+unpack-swap-bytes+
             #:+unpack-lsb-first+ #:+unpack-row-length+
             #:+unpack-skip-rows+ #:+unpack-skip-pixels+
             #:+unpack-alignment+ #:+pack-swap-bytes+
             #:+pack-lsb-first+ #:+pack-row-length+
             #:+pack-skip-rows+ #:+pack-skip-pixels+
             #:+pack-alignment+ #:+map-color+ #:+map-stencil+
             #:+index-shift+ #:+index-offset+ #:+red-scale+
             #:+red-bias+ #:+zoom-x+ #:+zoom-y+ #:+green-scale+
             #:+green-bias+ #:+blue-scale+ #:+blue-bias+
             #:+alpha-scale+ #:+alpha-bias+ #:+depth-scale+
             #:+depth-bias+ #:+max-eval-order+ #:+max-lights+
             ...
             #:+max-client-attrib-stack-depth+ #:+subpixel-bits+
             #:+index-bits+ #:+red-bits+ #:+green-bits+ #:+blue-bits+
             #:+alpha-bits+ #:+depth-bits+ #:+stencil-bits+
             #:+accum-red-bits+ #:+accum-green-bits+
             #:+accum-blue-bits+ #:+accum-alpha-bits+
             #:+name-stack-depth+ #:+map1-grid-domain+
             #:+map1-grid-segments+ #:+map2-grid-domain+
             #:+map2-grid-segments+ #:+feedback-buffer-size+
             #:+feedback-buffer-type+ #:+selection-buffer-size+
             #:+polygon-offset-units+ #:+polygon-offset-factor+
             #:+texture-binding-1d+ #:+texture-binding-2d+
             #:+texture-binding-3d+ #:+vertex-array-size+
             #:+vertex-array-type+ #:+vertex-array-stride+
             #:+normal-array-type+ #:+normal-array-stride+
             #:+color-array-size+ #:+color-array-type+
             #:+color-array-stride+ #:+index-array-type+
             #:+index-array-stride+ #:+texture-coord-array-size+
             #:+texture-coord-array-type+
             #:+texture-coord-array-stride+
             #:+edge-flag-array-stride+ #:+light-model-color-control+
             #:+blend-color-ext+ #:+blend-equation-ext+
             ...
             #:+polygon-offset-bias-ext+ #:+texture-3d-binding-ext+
             #:+pack-skip-images-ext+ #:+pack-image-height-ext+
             #:+unpack-skip-images-ext+ #:+unpack-image-height-ext+
             #:+max-3d-texture-size-ext+ #:+vertex-array-count-ext+
             #:+normal-array-count-ext+ #:+color-array-count-ext+
             #:+index-array-count-ext+
             #:+texture-coord-array-count-ext+
             #:+edge-flag-array-count-ext+
             #:+detail-texture-2d-binding-sgis+
             #:+fog-func-points-sgis+ #:+max-fog-func-points-sgis+
             #:+generate-mipmap-hint-sgis+ #:+sample-buffers-sgis+
             ...
             #:+texture-clipmap-depth-sgix+ #:+texture-compare-sgix+
             ...
             #:+texture-max-clamp-r-sgix+ #:+texture-lod-bias-s-sgix+
             #:+texture-lod-bias-t-sgix+ #:+texture-lod-bias-r-sgix+
             #:+post-texture-filter-bias-sgix+
             #:+post-texture-filter-scale-sgix+
             #:+texture-border-color-nv+ #:+dont-care+ #:+fastest+
             #:+nicest+ #:+texture-multi-buffer-hint-sgix+
             #:+proxy-histogram-ext+ #:+replace+ #:+modulate+
             #:+single-color+ #:+separate-specular-color+
             #:+position+ #:+spot-direction+ #:+spot-exponent+
             #:+spot-cutoff+ #:+constant-attenuation+
             #:+linear-attenuation+ #:+quadratic-attenuation+
             #:+compile+ #:+compile-and-execute+ #:+2-bytes+
             #:+3-bytes+ #:+4-bytes+ #:+double-ext+
             #:+list-priority-sgix+ #:+clear+ #:+and+ #:+and-reverse+
             #:+copy+ #:+and-inverted+ #:+noop+ #:+xor+ #:+or+
             ...
             #:+resample-decimate-sgix+ #:+pixel-subsample-4444-sgix+
             ...
             #:+unsigned-int-10-10-10-2-ext+ #:+render+ #:+feedback+
             #:+select+ #:+1pass-sgis+ #:+2pass-0-sgis+
             #:+2pass-1-sgis+ #:+4pass-0-sgis+ #:+4pass-1-sgis+
             #:+4pass-2-sgis+ #:+4pass-3-sgis+ #:+flat+ #:+smooth+
             #:+keep+ #:+incr+ #:+decr+ #:+vendor+ #:+renderer+
             #:+version+ #:+extensions+ #:+s+ #:+t+ #:+r+ #:+q+
             #:+decal+ #:+replace-ext+ #:+texture-env-bias-sgix+
             #:+texture-env-mode+ #:+texture-env-color+
             #:+texture-env+ #:+filter4-sgis+ #:+eye-linear+
             #:+object-linear+ #:+sphere-map+
             #:+eye-distance-to-point-sgis+
             #:+object-distance-to-point-sgis+
             #:+eye-distance-to-line-sgis+
             #:+object-distance-to-line-sgis+ #:+texture-gen-mode+
             #:+object-plane+ #:+eye-plane+ #:+eye-point-sgis+
             #:+object-point-sgis+ #:+eye-line-sgis+
             #:+object-line-sgis+ #:+nearest+ #:+linear-detail-sgis+
             #:+linear-detail-alpha-sgis+
             #:+linear-detail-color-sgis+ #:+linear-sharpen-sgis+
             ...
             #:+linear-clipmap-nearest-sgix+ #:+proxy-texture-1d+
             #:+proxy-texture-2d+ #:+proxy-texture-3d-ext+
             #:+detail-texture-2d-sgis+ #:+proxy-texture-4d-sgis+
             #:+clamp+ #:+repeat+ #:+clamp-to-border-sgis+
             #:+clamp-to-edge-sgis+ #:+r3-g3-b2+ #:+alpha4+
             #:+alpha8+ #:+alpha12+ #:+alpha16+ #:+luminance4+
             #:+luminance8+ #:+luminance12+ #:+luminance16+
             #:+luminance4-alpha4+ #:+luminance6-alpha2+
             #:+luminance8-alpha8+ #:+luminance12-alpha4+
             #:+luminance12-alpha12+ #:+luminance16-alpha16+
             #:+intensity+ #:+intensity4+ #:+intensity8+
             #:+intensity12+ #:+intensity16+ #:+rgb4+ #:+rgb5+
             #:+rgb8+ #:+rgb10+ #:+rgb12+ #:+rgb16+ #:+rgba2+
             #:+rgba4+ #:+rgb5-a1+ #:+rgba8+ #:+rgb10-a2+ #:+rgba12+
             #:+rgba16+ #:+rgb2-ext+ #:+dual-alpha4-sgis+
             #:+dual-alpha8-sgis+ #:+dual-alpha12-sgis+
             ...
             #:+quad-alpha8-sgis+ #:+quad-luminance4-sgis+
             ...
             #:+luminance-alpha-icc-sgix+ #:+intensity16-icc-sgix+
             #:+v2f+ #:+v3f+ #:+c4ub-v2f+ #:+c4ub-v3f+ #:+c3f-v3f+
             #:+n3f-v3f+ #:+c4f-n3f-v3f+ #:+t2f-v3f+ #:+t4f-v4f+
             #:+t2f-c4ub-v3f+ #:+t2f-c3f-v3f+ #:+t2f-n3f-v3f+
             #:+t2f-c4f-n3f-v3f+ #:+t4f-c4f-n3f-v4f+
             #:+clip-distance0+ #:+clip-distance1+ #:+clip-distance2+
             #:+clip-distance3+ #:+clip-distance4+ #:+clip-distance5+
             #:+clip-distance6+ #:+clip-distance7+ #:load-version_1_2
             #:load-version_1_3 #:load-version_1_4 #:load-version_1_5
             #:load-version_2_0 #:load-version_2_1 #:translate-f
             #:translate-d #:scale-f #:scale-d #:rotate-f #:rotate-d
             #:push-matrix #:pop-matrix #:ortho #:mult-matrix-d
             #:mult-matrix-f #:matrix-mode #:load-matrix-d
             #:load-matrix-f #:load-identity #:frustum #:is-list
             #:get-tex-gen-iv #:get-tex-gen-fv #:get-tex-gen-dv
             #:get-tex-env-iv #:get-tex-env-fv #:get-polygon-stipple
             #:get-pixel-map-usv #:get-pixel-map-uiv
             #:get-pixel-map-fv #:get-material-iv #:get-material-fv
             #:get-map-iv #:get-map-fv #:get-map-dv #:get-light-iv
             #:get-light-fv #:get-clip-plane #:draw-pixels
             #:copy-pixels #:pixel-map-usv #:pixel-map-uiv
             #:pixel-map-fv #:pixel-transfer-i #:pixel-transfer-f
             #:pixel-zoom #:alpha-func #:eval-point-2 #:eval-mesh-2
             #:eval-point-1 #:eval-mesh-1 #:eval-coord-2fv
             #:eval-coord-2f #:eval-coord-2dv #:eval-coord-2d
             #:eval-coord-1fv #:eval-coord-1f #:eval-coord-1dv
             #:eval-coord-1d #:map-grid-2f #:map-grid-2d
             #:map-grid-1f #:map-grid-1d #:map-2f #:map-2d #:map-1f
             #:map-1d #:push-attrib #:pop-attrib #:accum #:index-mask
             #:clear-index #:clear-accum #:push-name #:pop-name
             #:pass-through #:load-name #:init-names #:render-mode
             #:select-buffer #:feedback-buffer #:tex-gen-iv
             #:tex-gen-i #:tex-gen-fv #:tex-gen-f #:tex-gen-dv
             #:tex-gen-d #:tex-env-iv #:tex-env-i #:tex-env-fv
             #:tex-env-f #:shade-model #:polygon-stipple
             #:material-iv #:material-i #:material-fv #:material-f
             #:line-stipple #:light-model-iv #:light-model-i
             #:light-model-fv #:light-model-f #:light-iv #:light-i
             #:light-fv #:light-f #:fog-iv #:fog-i #:fog-fv #:fog-f
             #:color-material #:clip-plane #:vertex-4sv #:vertex-4s
             #:vertex-4iv #:vertex-4i #:vertex-4fv #:vertex-4f
             #:vertex-4dv #:vertex-4d #:vertex-3sv #:vertex-3s
             #:vertex-3iv #:vertex-3i #:vertex-3fv #:vertex-3f
             #:vertex-3dv #:vertex-3d #:vertex-2sv #:vertex-2s
             #:vertex-2iv #:vertex-2i #:vertex-2fv #:vertex-2f
             #:vertex-2dv #:vertex-2d #:tex-coord-4sv #:tex-coord-4s
             #:tex-coord-4iv #:tex-coord-4i #:tex-coord-4fv
             #:tex-coord-4f #:tex-coord-4dv #:tex-coord-4d
             #:tex-coord-3sv #:tex-coord-3s #:tex-coord-3iv
             #:tex-coord-3i #:tex-coord-3fv #:tex-coord-3f
             #:tex-coord-3dv #:tex-coord-3d #:tex-coord-2sv
             #:tex-coord-2s #:tex-coord-2iv #:tex-coord-2i
             #:tex-coord-2fv #:tex-coord-2f #:tex-coord-2dv
             #:tex-coord-2d #:tex-coord-1sv #:tex-coord-1s
             #:tex-coord-1iv #:tex-coord-1i #:tex-coord-1fv
             #:tex-coord-1f #:tex-coord-1dv #:tex-coord-1d #:rect-sv
             #:rect-s #:rect-iv #:rect-i #:rect-fv #:rect-f #:rect-dv
             #:rect-d #:raster-pos-4sv #:raster-pos-4s
             #:raster-pos-4iv #:raster-pos-4i #:raster-pos-4fv
             #:raster-pos-4f #:raster-pos-4dv #:raster-pos-4d
             #:raster-pos-3sv #:raster-pos-3s #:raster-pos-3iv
             #:raster-pos-3i #:raster-pos-3fv #:raster-pos-3f
             #:raster-pos-3dv #:raster-pos-3d #:raster-pos-2sv
             #:raster-pos-2s #:raster-pos-2iv #:raster-pos-2i
             #:raster-pos-2fv #:raster-pos-2f #:raster-pos-2dv
             #:raster-pos-2d #:normal-3sv #:normal-3s #:normal-3iv
             #:normal-3i #:normal-3fv #:normal-3f #:normal-3dv
             #:normal-3d #:normal-3bv #:normal-3b #:index-sv
             #:index-s #:index-iv #:index-i #:index-fv #:index-f
             #:index-dv #:index-d #:end #:edge-flagv #:edge-flag
             #:color-4usv #:color-4us #:color-4uiv #:color-4ui
             #:color-4ubv #:color-4ub #:color-4sv #:color-4s
             #:color-4iv #:color-4i #:color-4fv #:color-4f
             #:color-4dv #:color-4d #:color-4bv #:color-4b
             #:color-3usv #:color-3us #:color-3uiv #:color-3ui
             #:color-3ubv #:color-3ub #:color-3sv #:color-3s
             #:color-3iv #:color-3i #:color-3fv #:color-3f
             #:color-3dv #:color-3d #:color-3bv #:color-3b #:bitmap
             #:begin #:list-base #:gen-lists #:delete-lists
             #:call-lists #:call-list #:end-list #:new-list
             #:push-client-attrib #:pop-client-attrib
             #:prioritize-textures #:are-textures-resident
             #:vertex-pointer #:tex-coord-pointer #:normal-pointer
             #:interleaved-arrays #:index-pointer
             #:enable-client-state #:edge-flag-pointer
             #:disable-client-state #:color-pointer #:array-element
             #:reset-minmax #:reset-histogram #:minmax #:histogram
             #:get-minmax-parameter-iv #:get-minmax-parameter-fv
             #:get-minmax #:get-histogram-parameter-iv
             #:get-histogram-parameter-fv #:get-histogram
             #:separable-filter-2d #:get-separable-filter
             #:get-convolution-parameter-iv
             #:get-convolution-parameter-fv #:get-convolution-filter
             ...
             #:convolution-filter-1d #:copy-color-sub-table
             #:color-sub-table #:get-color-table-parameter-iv
             #:get-color-table-parameter-fv #:get-color-table
             #:copy-color-table #:color-table-parameter-iv
             #:color-table-parameter-fv #:color-table #:tex-image-3d
             #:mult-transpose-matrix-d #:mult-transpose-matrix-f
             #:load-transpose-matrix-d #:load-transpose-matrix-f
             #:multi-tex-coord-4sv #:multi-tex-coord-4s
             #:multi-tex-coord-4iv #:multi-tex-coord-4i
             #:multi-tex-coord-4fv #:multi-tex-coord-4f
             #:multi-tex-coord-4dv #:multi-tex-coord-4d
             #:multi-tex-coord-3sv #:multi-tex-coord-3s
             #:multi-tex-coord-3iv #:multi-tex-coord-3i
             #:multi-tex-coord-3fv #:multi-tex-coord-3f
             #:multi-tex-coord-3dv #:multi-tex-coord-3d
             #:multi-tex-coord-2sv #:multi-tex-coord-2s
             #:multi-tex-coord-2iv #:multi-tex-coord-2i
             #:multi-tex-coord-2fv #:multi-tex-coord-2f
             #:multi-tex-coord-2dv #:multi-tex-coord-2d
             #:multi-tex-coord-1sv #:multi-tex-coord-1s
             #:multi-tex-coord-1iv #:multi-tex-coord-1i
             #:multi-tex-coord-1fv #:multi-tex-coord-1f
             #:multi-tex-coord-1dv #:multi-tex-coord-1d
             #:client-active-texture #:window-pos-3sv #:window-pos-3s
             #:window-pos-3iv #:window-pos-3i #:window-pos-3fv
             #:window-pos-3f #:window-pos-3dv #:window-pos-3d
             #:window-pos-2sv #:window-pos-2s #:window-pos-2iv
             #:window-pos-2i #:window-pos-2fv #:window-pos-2f
             #:window-pos-2dv #:window-pos-2d
             #:secondary-color-pointer #:secondary-color-3usv
             #:secondary-color-3us #:secondary-color-3uiv
             #:secondary-color-3ui #:secondary-color-3ubv
             #:secondary-color-3ub #:secondary-color-3sv
             #:secondary-color-3s #:secondary-color-3iv
             #:secondary-color-3i #:secondary-color-3fv
             #:secondary-color-3f #:secondary-color-3dv
             #:secondary-color-3d #:secondary-color-3bv
             #:secondary-color-3b #:fog-coord-pointer #:fog-coord-dv
             #:fog-coord-d #:fog-coord-fv #:fog-coord-f
             #:vertex-attrib-4usv #:vertex-attrib-4uiv
             #:vertex-attrib-4ubv #:vertex-attrib-4sv
             #:vertex-attrib-4s #:vertex-attrib-4iv
             #:vertex-attrib-4fv #:vertex-attrib-4f
             #:vertex-attrib-4dv #:vertex-attrib-4d
             #:vertex-attrib-4bv #:vertex-attrib-4n-usv
             #:vertex-attrib-4n-uiv #:vertex-attrib-4n-ubv
             #:vertex-attrib-4n-ub #:vertex-attrib-4n-sv
             #:vertex-attrib-4n-iv #:vertex-attrib-4n-bv
             #:vertex-attrib-3sv #:vertex-attrib-3s
             #:vertex-attrib-3fv #:vertex-attrib-3f
             #:vertex-attrib-3dv #:vertex-attrib-3d
             #:vertex-attrib-2sv #:vertex-attrib-2s
             #:vertex-attrib-2fv #:vertex-attrib-2f
             #:vertex-attrib-2dv #:vertex-attrib-2d
             #:vertex-attrib-1sv #:vertex-attrib-1s
             #:vertex-attrib-1fv #:vertex-attrib-1f
             #:vertex-attrib-1dv #:vertex-attrib-1d
             #:vertex-attrib-i-4usv #:vertex-attrib-i-4ubv
             #:vertex-attrib-i-4sv #:vertex-attrib-i-4bv
             #:vertex-attrib-i-4uiv #:vertex-attrib-i-3uiv
             #:vertex-attrib-i-2uiv #:vertex-attrib-i-1uiv
             #:vertex-attrib-i-4iv #:vertex-attrib-i-3iv
             #:vertex-attrib-i-2iv #:vertex-attrib-i-1iv
             #:vertex-attrib-i-4ui #:vertex-attrib-i-3ui
             #:vertex-attrib-i-2ui #:vertex-attrib-i-1ui
             #:vertex-attrib-i-4i #:vertex-attrib-i-3i
             #:vertex-attrib-i-2i #:vertex-attrib-i-1i
             #:load-version_3_0 #:load-version_3_1 #:load-version_3_2
             #:load-version_3_3 #:load-version_4_0
             #:+max-elements-vertices+ #:+max-elements-indices+
             #:+current-program+ #:+point-sprite-coord-origin+
             #:+lower-left+ #:+upper-left+ #:+stencil-back-ref+
             #:+stencil-back-value-mask+ #:+stencil-back-writemask+
             #:+pixel-pack-buffer+ #:+pixel-unpack-buffer+
             ...
             #:+framebuffer-undefined-oes+ #:+r16f-ext+ #:+rg16f-ext+
             #:+rg-ext+ #:+r8-ext+ #:+rg8-ext+
             #:+max-renderbuffer-size-oes+ #:+compressed-alpha+
             #:+compressed-luminance+ #:+compressed-luminance-alpha+
             #:+compressed-intensity+ #:+compressed-rgb+
             #:+compressed-rgba+ #:+texture-compression-hint+
             #:+texture-compressed-image-size+
             #:+num-compressed-texture-formats+
             #:+compressed-texture-formats+ #:+depth24-stencil8-oes+
             #:+unsigned-normalized-ext+ #:+framebuffer-binding-oes+
             #:+renderbuffer-binding-oes+
             #:+framebuffer-binding-angle+
             #:+renderbuffer-binding-angle+
             #:+read-framebuffer-angle+ #:+draw-framebuffer-angle+
             #:+renderbuffer-samples-angle+ #:+color-attachment0-nv+
             ...
             #:+color-attachment15-nv+ #:+max-color-attachments-nv+
             ...
             #:+color-attachment0-oes+ #:+depth-attachment-oes+
             #:+stencil-attachment-oes+ #:+framebuffer-oes+
             #:+renderbuffer-oes+ #:+renderbuffer-width-oes+
             ...
             #:+renderbuffer-stencil-size-oes+ #:+stencil-index1-oes+
             #:+stencil-index4-oes+ #:+stencil-index8-oes+
             #:+framebuffer-incomplete-multisample-angle+
             #:+max-samples-angle+
             #:+framebuffer-incomplete-multisample-nv+
             #:+max-samples-nv+ #:+texture-gen-str-oes+
             #:+half-float-oes+ #:+rgb565-oes+ #:+major-version+
             #:+minor-version+ #:+num-extensions+ #:+context-flags+
             #:+compressed-red+ #:+compressed-rg+
             #:+context-flag-forward-compatible-bit+ #:+rgba32f+
             #:+rgba16f+ #:+rgb16f+ #:+vertex-attrib-array-integer+
             #:+max-array-texture-layers+
             #:+min-program-texel-offset+
             #:+max-program-texel-offset+ #:+clamp-read-color+
             #:+fixed-only+ #:+proxy-texture-1d-array+
             ...
             #:+sampler-cube-shadow+ #:+unsigned-int-vec2+
             #:+unsigned-int-vec3+ #:+unsigned-int-vec4+
             #:+int-sampler-1d+ #:+int-sampler-2d+ #:+int-sampler-3d+
             #:+int-sampler-cube+ #:+int-sampler-1d-array+
             #:+int-sampler-2d-array+ #:+unsigned-int-sampler-1d+
             #:+unsigned-int-sampler-2d+ #:+unsigned-int-sampler-3d+
             #:+unsigned-int-sampler-cube+
             #:+unsigned-int-sampler-1d-array+
             #:+unsigned-int-sampler-2d-array+ #:+query-wait+
             #:+query-no-wait+ #:+query-by-region-wait+
             ...
             #:+shader-binary-viv+ #:+sampler-buffer+
             ...
             #:+max-rectangle-texture-size+ #:+primitive-restart+
             #:+primitive-restart-index+ #:+context-core-profile-bit+
             ...
             #:+geometry-shader+ #:+geometry-vertices-out+
             ...
             #:get-tex-parameter-fv #:get-tex-image #:get-string
             #:get-integerv #:get-floatv #:get-error #:get-doublev
             #:get-booleanv #:read-pixels #:read-buffer
             #:pixel-store-i #:pixel-store-f #:depth-func
             #:stencil-op #:stencil-func #:logic-op #:blend-func
             #:flush #:finish #:enable #:disable #:depth-mask
             #:color-mask #:stencil-mask #:clear-depth
             #:clear-stencil #:clear-color #:clear #:draw-buffer
             #:tex-image-2d #:tex-image-1d #:tex-parameter-iv
             #:tex-parameter-i #:tex-parameter-fv #:tex-parameter-f
             #:scissor #:polygon-mode #:point-size #:line-width
             #:hint #:front-face #:cull-face #:index-ubv #:index-ub
             #:is-texture #:gen-textures #:delete-textures
             #:bind-texture #:tex-sub-image-2d #:tex-sub-image-1d
             #:copy-tex-sub-image-2d #:copy-tex-sub-image-1d
             #:copy-tex-image-2d #:copy-tex-image-1d #:polygon-offset
             #:get-pointerv #:draw-elements #:draw-arrays
             #:copy-tex-sub-image-3d #:tex-sub-image-3d
             #:draw-range-elements #:blend-equation #:blend-color
             #:get-compressed-tex-image #:compressed-tex-sub-image-1d
             ...
             #:sample-coverage #:active-texture #:point-parameter-iv
             #:point-parameter-i #:point-parameter-fv
             #:point-parameter-f #:multi-draw-elements
             #:multi-draw-arrays #:blend-func-separate
             #:get-buffer-pointerv #:get-buffer-parameter-iv
             #:unmap-buffer #:map-buffer #:get-buffer-sub-data
             #:buffer-sub-data #:buffer-data #:is-buffer
             #:gen-buffers #:delete-buffers #:bind-buffer
             #:get-query-object-uiv #:get-query-object-iv
             #:get-query-iv #:end-query #:begin-query #:is-query
             #:delete-queries #:gen-queries #:vertex-attrib-pointer
             #:validate-program #:uniform-matrix-4fv
             ...
             #:uniform-3i #:uniform-2i #:uniform-1i #:uniform-4f
             #:uniform-3f #:uniform-2f #:uniform-1f #:use-program
             #:shader-source #:link-program #:is-shader #:is-program
             #:get-vertex-attrib-pointerv #:get-vertex-attrib-iv
             #:get-vertex-attrib-fv #:get-vertex-attrib-dv
             #:get-uniform-iv #:get-uniform-fv #:get-uniform-location
             #:get-shader-source #:get-shader-info-log
             #:get-shader-iv #:get-program-info-log #:get-program-iv
             #:get-attrib-location #:get-attached-shaders
             #:get-active-uniform #:get-active-attrib
             #:enable-vertex-attrib-array
             #:disable-vertex-attrib-array #:detach-shader
             #:delete-shader #:delete-program #:create-shader
             #:create-program #:compile-shader #:bind-attrib-location
             #:attach-shader #:stencil-mask-separate
             #:stencil-func-separate #:stencil-op-separate
             #:draw-buffers #:blend-equation-separate
             #:uniform-matrix-4x-3fv #:uniform-matrix-3x-4fv
             #:uniform-matrix-4x-2fv #:uniform-matrix-2x-4fv
             #:uniform-matrix-3x-2fv #:uniform-matrix-2x-3fv
             #:get-string-i #:clear-bufferf-i #:clear-buffer-fv
             #:clear-buffer-uiv #:clear-buffer-iv
             #:get-tex-parameter-i-uiv #:get-tex-parameter-i-iv
             #:tex-parameter-i-uiv #:tex-parameter-i-iv
             #:uniform-4uiv #:uniform-3uiv #:uniform-2uiv
             #:uniform-1uiv #:uniform-4ui #:uniform-3ui #:uniform-2ui
             #:uniform-1ui #:get-frag-data-location
             #:bind-frag-data-location #:get-uniform-uiv
             #:get-vertex-attrib-i-uiv #:get-vertex-attrib-i-iv
             #:vertex-attrib-ipointer #:end-conditional-render
             #:begin-conditional-render #:clamp-color
             #:get-transform-feedback-varying
             #:transform-feedback-varyings #:bind-buffer-base
             #:bind-buffer-range #:end-transform-feedback
             #:begin-transform-feedback #:is-enabled-i #:disable-i
             #:enable-i #:get-integeri_v #:get-booleani_v
             #:color-mask-i #:primitive-restart-index #:tex-buffer
             #:draw-elements-instanced #:draw-arrays-instanced
             #:framebuffer-texture #:get-buffer-parameteri-64v
             #:get-integer-64i_v #:vertex-attrib-divisor
             ...
             #:+mat-diffuse-bit-pgi+ #:+mat-emission-bit-pgi+
             #:+mat-color-indexes-bit-pgi+ #:+mat-shininess-bit-pgi+
             ...
             #:+pack-compressed-block-size+ #:+compute-program-nv+
             ...
             #:+scaled-resolve-nicest-ext+ #:+rgb10-a2ui+
             #:+depth-clamp-near-amd+ #:+depth-clamp-far-amd+
             #:+factor-min-amd+ #:+factor-max-amd+ #:+alpha-snorm+
             #:+luminance-snorm+ #:+luminance-alpha-snorm+
             #:+intensity-snorm+ #:+alpha8-snorm+
             #:+luminance8-snorm+ #:+luminance8-alpha8-snorm+
             #:+intensity8-snorm+ #:+alpha16-snorm+
             #:+luminance16-snorm+ #:+luminance16-alpha16-snorm+
             #:+intensity16-snorm+ #:+red-snorm+ #:+rg-snorm+
             #:+rgb-snorm+ #:+rgba-snorm+ #:+r8-snorm+ #:+rg8-snorm+
             #:+rgb8-snorm+ #:+rgba8-snorm+ #:+r16-snorm+
             #:+rg16-snorm+ #:+rgb16-snorm+ #:+rgba16-snorm+
             #:+signed-normalized+ #:+texture-binding-cube-map-array+
             ...
             #:+texture-swizzle-rgba-ext+ #:+texture-swizzle-r+
             #:+texture-swizzle-g+ #:+texture-swizzle-b+
             #:+texture-swizzle-a+ #:+texture-swizzle-rgba+
             #:+color-samples-nv+ #:+compressed-red-rgtc1-ext+
             ...
             #:+srgb8-nv+ #:+sluminance-alpha-nv+
             #:+sluminance8-alpha8-nv+ #:+sluminance-nv+
             #:+sluminance8-nv+ #:+compressed-srgb-s3tc-dxt1-nv+
             #:+compressed-srgb-alpha-s3tc-dxt1-nv+
             #:+compressed-srgb-alpha-s3tc-dxt3-nv+
             #:+compressed-srgb-alpha-s3tc-dxt5-nv+ #:+srgb-ext+
             #:+srgb8-ext+ #:+srgb-alpha-ext+ #:+srgb8-alpha8-ext+
             #:+sluminance-alpha-ext+ #:+sluminance8-alpha8-ext+
             #:+sluminance-ext+ #:+sluminance8-ext+
             ...
             #:+srgb8+ #:+srgb-alpha+ #:+srgb8-alpha8+
             #:+sluminance-alpha+ #:+sluminance8-alpha8+
             #:+sluminance+ #:+sluminance8+ #:+compressed-srgb+
             #:+compressed-srgb-alpha+ #:+compressed-sluminance+
             #:+compressed-sluminance-alpha+ #:+rgb9-e5-ext+
             #:+unsigned-int-5-9-9-9-rev-ext+
             #:+texture-shared-size-ext+ #:+r11f-g11f-b10f-ext+
             #:+unsigned-int-10f-11f-11f-rev-ext+
             #:+rgba-signed-components-ext+
             #:+any-samples-passed-ext+ #:+any-samples-passed+
             ...
             #:+sync-object-apple+ #:+sampler-2d-rect+
             #:+sampler-2d-rect-shadow+ #:+fragment-shader+
             #:+fragment-shader-arb+ #:+vertex-shader+
             ...
             #:+float-vec2+ #:+float-vec3+ #:+float-vec4+
             #:+int-vec2+ #:+int-vec3+ #:+int-vec4+ #:+bool+
             #:+bool-vec2+ #:+bool-vec3+ #:+bool-vec4+ #:+float-mat2+
             #:+float-mat3+ #:+float-mat4+ #:+sampler-1d+
             #:+sampler-2d+ #:+sampler-3d+ #:+sampler-cube+
             #:+sampler-1d-shadow+ #:+sampler-2d-shadow+
             #:+float-mat2x3+ #:+float-mat2x4+ #:+float-mat3x2+
             #:+float-mat3x4+ #:+float-mat4x2+ #:+float-mat4x3+
             #:+delete-status+ #:+compile-status+ #:+link-status+
             #:+validate-status+ #:+info-log-length+
             #:+attached-shaders+ #:+active-uniforms+
             ...
             #:+shading-language-version-arb+ #:+program-object-ext+
             #:+shader-object-ext+ #:+sampler-3d-oes+
             #:+sampler-2d-shadow-ext+
             #:+fragment-shader-derivative-hint-oes+
             #:+texture-srgb-decode-ext+ #:+decode-ext+
             #:+skip-decode-ext+ #:+rgb-422-apple+
             #:+pack-row-bytes-apple+ #:+unpack-row-bytes-apple+
             #:+aux-depth-stencil-apple+ #:+color-float-apple+
             ...
             #:+format-subsample-244-244-oml+ #:+interlace-oml+
             #:+interlace-read-oml+ #:+max-program-if-depth-nv+
             #:+max-program-loop-depth-nv+
             #:+max-program-loop-count-nv+ #:+etc1-srgb8-nv+
             ...
             #:+texture-unsigned-remap-mode-nv+ #:+float-r-nv+
             #:+float-rg-nv+ #:+float-rgb-nv+ #:+float-rgba-nv+
             #:+float-r16-nv+ #:+float-r32-nv+ #:+float-rg16-nv+
             #:+float-rg32-nv+ #:+float-rgb16-nv+ #:+float-rgb32-nv+
             #:+float-rgba16-nv+ #:+float-rgba32-nv+
             #:+texture-float-components-nv+
             #:+float-clear-color-value-nv+ #:+float-rgba-mode-nv+
             #:+max-texture-coords+ #:+max-texture-image-units+
             #:+depth-stencil-to-rgba-nv+
             #:+depth-stencil-to-bgra-nv+ #:+point-sprite+
             #:+coord-replace+ #:+offset-projective-texture-2d-nv+
             ...
             #:+luminance-alpha-float16-apple+ #:+rgba16f-ext+
             #:+rgb16f-ext+ #:+rgba-float32-ati+ #:+rgb-float32-ati+
             #:+alpha-float32-ati+ #:+intensity-float32-ati+
             ...
             #:+luminance-alpha-float16-ati+ #:+texture-red-type-arb+
             #:+texture-green-type-arb+ #:+texture-blue-type-arb+
             ...
             #:+luminance-alpha32f-arb+ #:+rgba16f-arb+
             ...
             #:+program-binary-formats-oes+ #:+vbo-free-memory-ati+
             #:+texture-free-memory-ati+
             #:+renderbuffer-free-memory-ati+ #:+debug-object-mesa+
             #:+debug-print-mesa+ #:+debug-assert-mesa+
             ...
             #:+texture-2d-stack-binding-mesax+ #:+pack-invert-mesa+
             #:+ycbcr-mesa+ #:+trace-all-bits-mesa+
             #:+trace-operations-bit-mesa+
             #:+trace-primitives-bit-mesa+ #:+trace-arrays-bit-mesa+
             #:+trace-textures-bit-mesa+ #:+trace-pixels-bit-mesa+
             #:+trace-errors-bit-mesa+ #:+trace-mask-mesa+
             #:+trace-name-mesa+ #:+depth-stencil-mesa+
             #:+unsigned-int-24-8-mesa+
             #:+unsigned-int-8-24-rev-mesa+
             #:+unsigned-short-15-1-mesa+
             #:+unsigned-short-1-15-rev-mesa+ #:+modulate-add-ati+
             ...
             #:+z400-binary-amd+ #:+program-binary-length-oes+
             ...
             #:+dependent-gb-texture-2d-nv+ #:+dot-product-nv+
             ...
             #:+hilo-nv+ #:+dsdt-nv+ #:+dsdt-mag-nv+
             #:+dsdt-mag-vib-nv+ #:+hilo16-nv+ #:+signed-hilo-nv+
             #:+signed-hilo16-nv+ #:+signed-rgba-nv+
             ...
             #:+signed-alpha8-nv+ #:+signed-intensity-nv+
             #:+signed-intensity8-nv+ #:+dsdt8-nv+ #:+dsdt8-mag8-nv+
             #:+dsdt8-mag8-intensity8-nv+
             #:+signed-rgb-unsigned-alpha-nv+
             #:+signed-rgb8-unsigned-alpha8-nv+ #:+hi-scale-nv+
             #:+lo-scale-nv+ #:+ds-scale-nv+ #:+dt-scale-nv+
             #:+magnitude-scale-nv+ #:+vibrance-scale-nv+
             #:+hi-bias-nv+ #:+lo-bias-nv+ #:+ds-bias-nv+
             #:+dt-bias-nv+ #:+magnitude-bias-nv+
             #:+vibrance-bias-nv+ #:+texture-border-values-nv+
             #:+texture-hi-size-nv+ #:+texture-lo-size-nv+
             #:+texture-ds-size-nv+ #:+texture-dt-size-nv+
             #:+texture-mag-size-nv+ #:+max-program-patch-attribs-nv+
             ...
             #:+compressed-rgba-fxt1-3dfx+ #:+dot3-rgba-img+
             ...
             #:+vertex-attrib-array-pointer+ #:+depth-clamp-nv+
             #:+depth-clamp+ #:+slice-accum-sun+
             #:+unsigned-short-8-8-mesa+
             #:+unsigned-short-8-8-rev-mesa+ #:+ycbcr-422-apple+
             ...
             #:+operand0-rgb-ext+ #:+operand1-rgb-ext+
             #:+operand2-rgb-ext+ #:+operand0-alpha-ext+
             #:+operand1-alpha-ext+ #:+operand2-alpha-ext+
             #:+src0-rgb+ #:+src1-rgb+ #:+src2-rgb+ #:+src0-alpha+
             #:+src2-alpha+ #:+combine+ #:+combine-rgb+
             #:+combine-alpha+ #:+rgb-scale+ #:+add-signed+
             #:+interpolate+ #:+constant+ #:+previous+
             ...
             #:+interlace-read-ingr+ #:+red-min-clamp-ingr+
             ...
             #:+max-cube-map-texture-size-oes+ #:+normal-map-arb+
             #:+reflection-map-arb+ #:+texture-cube-map-arb+
             ...
             #:+max-cube-map-texture-size-arb+ #:+normal-map-nv+
             #:+reflection-map-nv+ #:+normal-map-ext+
             #:+reflection-map-ext+ #:+texture-cube-map-ext+
             ...
             #:+max-cube-map-texture-size-ext+ #:+incr-wrap-oes+
             #:+decr-wrap-oes+ #:+incr-wrap-ext+ #:+decr-wrap-ext+
             #:+max-shininess-nv+ #:+max-spot-exponent-nv+
             #:+texture-filter-control+ #:+texture-lod-bias+
             #:+texture-max-anisotropy-ext+
             #:+max-texture-max-anisotropy-ext+
             #:+max-texture-lod-bias-ext+
             #:+texture-filter-control-ext+ #:+texture-lod-bias-ext+
             #:+depth-stencil-oes+ #:+unsigned-int-24-8-oes+
             #:+max-texture-lod-bias+ #:+depth-stencil-nv+
             #:+unsigned-int-24-8-nv+ #:+depth-stencil-ext+
             #:+unsigned-int-24-8-ext+ #:+depth24-stencil8-ext+
             #:+texture-stencil-size-ext+ #:+texture-rectangle-nv+
             ...
             #:+max-rectangle-texture-size-arb+ #:+combine-arb+
             #:+combine-rgb-arb+ #:+combine-alpha-arb+
             #:+source0-rgb-arb+ #:+source1-rgb-arb+
             #:+source2-rgb-arb+ #:+source0-alpha-arb+
             #:+source1-alpha-arb+ #:+source2-alpha-arb+
             #:+operand0-rgb-arb+ #:+operand1-rgb-arb+
             #:+operand2-rgb-arb+ #:+operand0-alpha-arb+
             #:+operand1-alpha-arb+ #:+operand2-alpha-arb+
             #:+rgb-scale-arb+ #:+add-signed-arb+ #:+interpolate-arb+
             #:+subtract-arb+ #:+constant-arb+ #:+primary-color-arb+
             ...
             #:+compressed-rgba-s3tc-dxt5-ext+ #:+rgb-s3tc+
             ...
             #:+ycrcba-sgix+ #:+text-fragment-shader-ati+
             #:+light-model-color-control-ext+ #:+single-color-ext+
             #:+separate-specular-color-ext+ #:+iui-v2f-ext+
             #:+iui-v3f-ext+ #:+iui-n3f-v2f-ext+ #:+iui-n3f-v3f-ext+
             #:+t2f-iui-v2f-ext+ #:+t2f-iui-v3f-ext+
             #:+t2f-iui-n3f-v2f-ext+ #:+t2f-iui-n3f-v3f-ext+
             #:+depth-component16-oes+ #:+depth-component24-oes+
             #:+depth-component32-oes+ #:+compare-ref-to-texture+
             #:+texture-compare-mode+ #:+texture-compare-func+
             #:+compare-r-to-texture+ #:+depth-component16-arb+
             #:+depth-component24-arb+ #:+depth-component32-arb+
             ...
             #:+luminance4-alpha4-ext+ #:+luminance6-alpha2-ext+
             #:+luminance8-alpha8-ext+ #:+luminance12-alpha4-ext+
             #:+luminance12-alpha12-ext+ #:+luminance16-alpha16-ext+
             #:+intensity-ext+ #:+intensity4-ext+ #:+intensity8-ext+
             #:+intensity12-ext+ #:+intensity16-ext+ #:+rgb4-ext+
             #:+rgb5-ext+ #:+rgb8-ext+ #:+rgb10-ext+ #:+rgb12-ext+
             #:+rgb16-ext+ #:+rgba2-ext+ #:+rgba4-ext+
             #:+rgb5-a1-ext+ #:+rgba8-ext+ #:+rgb10-a2-ext+
             #:+rgba12-ext+ #:+rgba16-ext+ #:+texture-red-size-ext+
             #:+texture-green-size-ext+ #:+texture-blue-size-ext+
             #:+texture-alpha-size-ext+
             #:+texture-luminance-size-ext+
             #:+texture-intensity-size-ext+ #:+proxy-texture-1d-ext+
             #:+proxy-texture-2d-ext+ #:+unsigned-byte-2-3-3-rev-ext+
             ...
             #:+unsigned-int-2-10-10-10-rev-ext+ #:+convolution-1d+
             #:+convolution-2d+ #:+separable-2d+
             #:+convolution-border-mode+ #:+convolution-filter-scale+
             #:+convolution-filter-bias+ #:+reduce+
             #:+convolution-format+ #:+convolution-width+
             ...
             #:+post-convolution-alpha-bias+ #:+func-subtract+
             #:+func-reverse-subtract+ #:+func-add-oes+
             #:+blend-equation-oes+ #:+func-subtract-oes+
             #:+func-reverse-subtract-oes+ #:+constant-color+
             #:+one-minus-constant-color+ #:+constant-alpha+
             #:+one-minus-constant-alpha+ #:+blend-color+
             #:+half-float-arb+ #:+depth-pass-instrument-sgix+
             ...
             #:+vertex-attrib-array-long+ #:+compressed-rgb8-etc2+
             ...
             #:+compressed-r11-eac+ #:+compressed-signed-r11-eac+
             #:+compressed-rg11-eac+ #:+compressed-signed-rg11-eac+
             #:+primitive-restart-fixed-index+
             #:+any-samples-passed-conservative+
             #:+max-element-index+ #:+max-uniform-locations+
             #:+depth-stencil-texture-mode+ #:+multisample-3dfx+
             #:+sample-buffers-3dfx+ #:+samples-3dfx+
             #:+multisample-bit-3dfx+ #:+texture0+ #:+texture1+
             #:+texture2+ #:+texture3+ #:+texture4+ #:+texture5+
             #:+texture6+ #:+texture7+ #:+texture8+ #:+texture9+
             ...
             #:+texture26+ #:+texture27+ #:+texture28+ #:+texture29+
             #:+texture30+ #:+texture31+
             #:+transpose-modelview-matrix+
             #:+transpose-projection-matrix+
             #:+transpose-texture-matrix+ #:+transpose-color-matrix+
             #:+texture2-arb+ #:+texture3-arb+ #:+texture4-arb+
             #:+texture5-arb+ #:+texture6-arb+ #:+texture7-arb+
             #:+texture8-arb+ #:+texture9-arb+ #:+texture10-arb+
             #:+texture11-arb+ #:+texture12-arb+ #:+texture13-arb+
             #:+texture14-arb+ #:+texture15-arb+ #:+texture16-arb+
             #:+texture17-arb+ #:+texture18-arb+ #:+texture19-arb+
             #:+texture20-arb+ #:+texture21-arb+ #:+texture22-arb+
             #:+texture23-arb+ #:+texture24-arb+ #:+texture25-arb+
             #:+texture26-arb+ #:+texture27-arb+ #:+texture28-arb+
             #:+texture29-arb+ #:+texture30-arb+ #:+texture31-arb+
             #:+active-texture-arb+ #:+client-active-texture-arb+
             #:+max-texture-units-arb+ #:multi-tex-coord-4sv-arb
             #:multi-tex-coord-4s-arb #:multi-tex-coord-4iv-arb
             #:multi-tex-coord-4i-arb #:multi-tex-coord-4fv-arb
             #:multi-tex-coord-4f-arb #:multi-tex-coord-4dv-arb
             #:multi-tex-coord-4d-arb #:multi-tex-coord-3sv-arb
             #:multi-tex-coord-3s-arb #:multi-tex-coord-3iv-arb
             #:multi-tex-coord-3i-arb #:multi-tex-coord-3fv-arb
             #:multi-tex-coord-3f-arb #:multi-tex-coord-3dv-arb
             #:multi-tex-coord-3d-arb #:multi-tex-coord-2sv-arb
             #:multi-tex-coord-2s-arb #:multi-tex-coord-2iv-arb
             #:multi-tex-coord-2i-arb #:multi-tex-coord-2fv-arb
             #:multi-tex-coord-2f-arb #:multi-tex-coord-2dv-arb
             #:multi-tex-coord-2d-arb #:multi-tex-coord-1sv-arb
             #:multi-tex-coord-1s-arb #:multi-tex-coord-1iv-arb
             #:multi-tex-coord-1i-arb #:multi-tex-coord-1fv-arb
             #:multi-tex-coord-1f-arb #:multi-tex-coord-1dv-arb
             #:multi-tex-coord-1d-arb #:client-active-texture-arb
             #:active-texture-arb #:load-arb_multitexture
             #:+subtract+ #:+transpose-modelview-matrix-arb+
             ...
             #:load-arb_transpose_matrix #:+multisample-arb+
             ...
             #:+sample-coverage-invert-arb+ #:+multisample-bit-arb+
             #:sample-coverage-arb #:load-arb_multisample
             ...
             #:load-arb_texture_compression #:+point-size-min-arb+
             #:+point-size-max-arb+ #:+point-fade-threshold-size-arb+
             #:+point-distance-attenuation-arb+
             #:point-parameter-fv-arb #:point-parameter-f-arb
             #:load-arb_point_parameters #:+max-vertex-units-oes+
             #:+weight-array-oes+ #:+weight-array-type-oes+
             #:+weight-array-stride-oes+ #:+weight-array-size-oes+
             #:+weight-array-pointer-oes+ #:+dot3-rgb+ #:+dot3-rgba+
             ...
             #:+modelview9-arb+ #:+modelview10-arb+
             #:+modelview11-arb+ #:+modelview12-arb+
             #:+modelview13-arb+ #:+modelview14-arb+
             #:+modelview15-arb+ #:+modelview16-arb+
             #:+modelview17-arb+ #:+modelview18-arb+
             #:+modelview19-arb+ #:+modelview20-arb+
             #:+modelview21-arb+ #:+modelview22-arb+
             #:+modelview23-arb+ #:+modelview24-arb+
             #:+modelview25-arb+ #:+modelview26-arb+
             #:+modelview27-arb+ #:+modelview28-arb+
             #:+modelview29-arb+ #:+modelview30-arb+
             #:+modelview31-arb+ #:vertex-blend-arb
             #:weight-pointer-arb #:weight-uiv-arb #:weight-usv-arb
             #:weight-ubv-arb #:weight-dv-arb #:weight-fv-arb
             #:weight-iv-arb #:weight-sv-arb #:weight-bv-arb
             #:load-arb_vertex_blend #:+matrix-palette-oes+
             ...
             #:+texture-depth-size+ #:+depth-texture-mode+
             #:+matrix-palette-arb+
             ...
             #:matrix-index-pointer-arb #:matrix-index-uiv-arb
             #:matrix-index-usv-arb #:matrix-index-ubv-arb
             #:current-palette-matrix-arb #:load-arb_matrix_palette
             #:window-pos-3sv-arb #:window-pos-3s-arb
             #:window-pos-3iv-arb #:window-pos-3i-arb
             #:window-pos-3fv-arb #:window-pos-3f-arb
             #:window-pos-3dv-arb #:window-pos-3d-arb
             #:window-pos-2sv-arb #:window-pos-2s-arb
             #:window-pos-2iv-arb #:window-pos-2i-arb
             #:window-pos-2fv-arb #:window-pos-2f-arb
             #:window-pos-2dv-arb #:window-pos-2d-arb
             #:load-arb_window_pos #:+color-sum-arb+
             #:+vertex-program-arb+
             ...
             #:+program-format-ascii-arb+ #:+program-format-arb+
             #:is-program-arb #:get-vertex-attrib-pointerv-arb
             ...
             #:program-env-parameter-4d-arb #:gen-programs-arb
             #:delete-programs-arb #:bind-program-arb
             #:program-string-arb #:disable-vertex-attrib-array-arb
             #:enable-vertex-attrib-array-arb
             #:vertex-attrib-pointer-arb #:vertex-attrib-4usv-arb
             #:vertex-attrib-4uiv-arb #:vertex-attrib-4ubv-arb
             #:vertex-attrib-4sv-arb #:vertex-attrib-4s-arb
             #:vertex-attrib-4iv-arb #:vertex-attrib-4fv-arb
             #:vertex-attrib-4f-arb #:vertex-attrib-4dv-arb
             #:vertex-attrib-4d-arb #:vertex-attrib-4bv-arb
             #:vertex-attrib-4n-usv-arb #:vertex-attrib-4n-uiv-arb
             #:vertex-attrib-4n-ubv-arb #:vertex-attrib-4n-ub-arb
             #:vertex-attrib-4n-sv-arb #:vertex-attrib-4n-iv-arb
             #:vertex-attrib-4n-bv-arb #:vertex-attrib-3sv-arb
             #:vertex-attrib-3s-arb #:vertex-attrib-3fv-arb
             #:vertex-attrib-3f-arb #:vertex-attrib-3dv-arb
             #:vertex-attrib-3d-arb #:vertex-attrib-2sv-arb
             #:vertex-attrib-2s-arb #:vertex-attrib-2fv-arb
             #:vertex-attrib-2f-arb #:vertex-attrib-2dv-arb
             #:vertex-attrib-2d-arb #:vertex-attrib-1sv-arb
             #:vertex-attrib-1s-arb #:vertex-attrib-1fv-arb
             #:vertex-attrib-1f-arb #:vertex-attrib-1dv-arb
             #:vertex-attrib-1d-arb #:load-arb_vertex_program
             ...
             #:+program-attribs-arb+ #:+max-program-attribs-arb+
             ...
             #:+transpose-current-matrix-arb+ #:+read-only+
             #:+write-only+ #:+read-write+ #:+buffer-access+
             #:+buffer-mapped+ #:+buffer-map-pointer+
             #:+write-only-oes+ #:+buffer-access-oes+
             #:+buffer-mapped-oes+ #:+buffer-map-pointer-oes+
             #:+array-buffer-arb+ #:+element-array-buffer-arb+
             ...
             #:+read-only-arb+ #:+write-only-arb+ #:+read-write-arb+
             #:+buffer-access-arb+ #:+buffer-mapped-arb+
             #:+buffer-map-pointer-arb+ #:get-buffer-pointerv-arb
             #:get-buffer-parameter-iv-arb #:unmap-buffer-arb
             #:map-buffer-arb #:get-buffer-sub-data-arb
             ...
             #:load-arb_occlusion_query #:+program-object-arb+
             #:+shader-object-arb+ #:+object-type-arb+
             #:+object-subtype-arb+ #:+float-vec2-arb+
             #:+float-vec3-arb+ #:+float-vec4-arb+ #:+int-vec2-arb+
             #:+int-vec3-arb+ #:+int-vec4-arb+ #:+bool-arb+
             ...
             #:get-shader-source-arb #:get-uniform-iv-arb
             #:get-uniform-fv-arb #:get-active-uniform-arb
             ...
             #:uniform-1f-arb #:validate-program-arb
             #:use-program-object-arb #:link-program-arb
             #:attach-object-arb #:create-program-object-arb
             #:compile-shader-arb #:shader-source-arb
             #:create-shader-object-arb #:detach-object-arb
             #:get-handle-arb #:delete-object-arb
             #:load-arb_shader_objects #:+vertex-shader-arb+
             ...
             #:bind-attrib-location-arb #:load-arb_vertex_shader
             #:+max-draw-buffers-arb+ #:+draw-buffer0-arb+
             #:+draw-buffer1-arb+ #:+draw-buffer2-arb+
             #:+draw-buffer3-arb+ #:+draw-buffer4-arb+
             #:+draw-buffer5-arb+ #:+draw-buffer6-arb+
             #:+draw-buffer7-arb+ #:+draw-buffer8-arb+
             #:+draw-buffer9-arb+ #:+draw-buffer10-arb+
             ...
             #:draw-arrays-instanced-arb #:load-arb_draw_instanced
             ...
             #:+framebuffer-default+ #:+framebuffer-undefined+
             #:+depth-stencil-attachment+ #:+max-renderbuffer-size+
             #:+depth-stencil+ #:+unsigned-int-24-8+
             #:+depth24-stencil8+ #:+texture-stencil-size+
             #:+texture-red-type+ #:+texture-green-type+
             #:+texture-blue-type+ #:+texture-alpha-type+
             #:+texture-depth-type+ #:+unsigned-normalized+
             #:+framebuffer-binding+ #:+draw-framebuffer-binding+
             #:+renderbuffer-binding+ #:+read-framebuffer+
             #:+draw-framebuffer+ #:+read-framebuffer-binding+
             ...
             #:+framebuffer-unsupported+ #:+max-color-attachments+
             #:+color-attachment0+ #:+color-attachment1+
             #:+color-attachment2+ #:+color-attachment3+
             #:+color-attachment4+ #:+color-attachment5+
             #:+color-attachment6+ #:+color-attachment7+
             #:+color-attachment8+ #:+color-attachment9+
             #:+color-attachment10+ #:+color-attachment11+
             #:+color-attachment12+ #:+color-attachment13+
             #:+color-attachment14+ #:+color-attachment15+
             #:+depth-attachment+ #:+stencil-attachment+
             #:+framebuffer+ #:+renderbuffer-width+
             #:+renderbuffer-height+ #:+renderbuffer-internal-format+
             #:+stencil-index1+ #:+stencil-index4+ #:+stencil-index8+
             #:+stencil-index16+ #:+renderbuffer-red-size+
             #:+renderbuffer-green-size+ #:+renderbuffer-blue-size+
             #:+renderbuffer-alpha-size+ #:+renderbuffer-depth-size+
             #:+renderbuffer-stencil-size+
             #:+framebuffer-incomplete-multisample+ #:+max-samples+
             #:+index+ #:+texture-luminance-type+
             #:+texture-intensity-type+ #:framebuffer-texture-layer
             #:renderbuffer-storage-multisample #:blit-framebuffer
             #:generate-mipmap
             #:get-framebuffer-attachment-parameter-iv
             #:framebuffer-renderbuffer #:framebuffer-texture-3d
             #:framebuffer-texture-2d #:framebuffer-texture-1d
             #:check-framebuffer-status #:gen-framebuffers
             #:delete-framebuffers #:bind-framebuffer
             #:is-framebuffer #:get-renderbuffer-parameter-iv
             #:renderbuffer-storage #:gen-renderbuffers
             #:delete-renderbuffers #:bind-renderbuffer
             #:is-renderbuffer #:load-arb_framebuffer_object
             ...
             #:+active-uniform-blocks+ #:+uniform-type+
             #:+uniform-block-index+ #:+uniform-offset+
             #:+uniform-array-stride+ #:+uniform-matrix-stride+
             ...
             #:+invalid-index+ #:uniform-block-binding
             #:get-active-uniform-block-name
             #:get-active-uniform-block-iv #:get-uniform-block-index
             #:get-active-uniform-name #:get-active-uniforms-iv
             #:get-uniform-indices #:load-arb_uniform_buffer_object
             #:+copy-read-buffer-binding+ #:+copy-read-buffer+
             #:+copy-write-buffer-binding+ #:+copy-write-buffer+
             #:copy-buffer-sub-data #:load-arb_copy_buffer
             ...
             #:provoking-vertex #:load-arb_provoking_vertex
             #:+max-server-wait-timeout-apple+ #:+object-type-apple+
             #:+sync-condition-apple+ #:+sync-status-apple+
             #:+sync-flags-apple+ #:+sync-fence-apple+
             #:+sync-gpu-commands-complete-apple+
             #:+unsignaled-apple+ #:+signaled-apple+
             #:+already-signaled-apple+ #:+timeout-expired-apple+
             #:+condition-satisfied-apple+ #:+wait-failed-apple+
             #:+sync-flush-commands-bit-apple+
             #:+timeout-ignored-apple+ #:+max-server-wait-timeout+
             #:+object-type+ #:+sync-condition+ #:+sync-status+
             #:+sync-flags+ #:+sync-fence+
             #:+sync-gpu-commands-complete+ #:+unsignaled+
             #:+signaled+ #:+already-signaled+ #:+timeout-expired+
             #:+condition-satisfied+ #:+wait-failed+
             #:+sync-flush-commands-bit+ #:+timeout-ignored+
             #:get-sync-iv #:get-integer-64v #:wait-sync
             #:client-wait-sync #:delete-sync #:is-sync #:fence-sync
             #:load-arb_sync #:+sample-position+ #:+sample-mask+
             ...
             #:+texture-samples+ #:+texture-fixed-sample-locations+
             ...
             #:+max-depth-texture-samples+ #:+max-integer-samples+
             #:sample-mask-i #:get-multisample-fv
             #:tex-image-3dmultisample #:tex-image-2dmultisample
             #:load-arb_texture_multisample
             #:blend-func-separate-i-arb #:blend-func-i-arb
             #:blend-equation-separate-i-arb #:blend-equation-i-arb
             #:load-arb_draw_buffers_blend #:+sample-shading-arb+
             #:+min-sample-shading-value-arb+
             #:min-sample-shading-arb #:load-arb_sample_shading
             #:+unsigned-int-10-10-10-2-oes+ #:+int-10-10-10-2-oes+
             #:+named-string-length-arb+ #:+named-string-type-arb+
             #:get-named-string-iv-arb #:get-named-string-arb
             #:is-named-string-arb #:compile-shader-include-arb
             #:delete-named-string-arb #:named-string-arb
             #:load-arb_shading_language_include #:+src1-color+
             #:+src1-alpha+ #:+one-minus-src1-color+
             #:+one-minus-src1-alpha+
             #:+max-dual-source-draw-buffers+ #:get-frag-data-index
             #:bind-frag-data-location-indexed
             #:load-arb_blend_func_extended #:+sampler-binding+
             #:get-sampler-parameter-i-uiv #:get-sampler-parameter-fv
             #:get-sampler-parameter-i-iv #:get-sampler-parameter-iv
             #:sampler-parameter-i-uiv #:sampler-parameter-i-iv
             #:sampler-parameter-fv #:sampler-parameter-f
             #:sampler-parameter-iv #:sampler-parameter-i
             #:bind-sampler #:is-sampler #:delete-samplers
             #:gen-samplers #:load-arb_sampler_objects
             #:+depth-component16-nonlinear-nv+ #:+time-elapsed+
             #:+timestamp+ #:get-query-objectui-64v
             #:get-query-objecti-64v #:query-counter
             #:load-arb_timer_query #:+int-2-10-10-10-rev+
             #:vertex-attrib-p-4uiv #:vertex-attrib-p-4ui
             #:vertex-attrib-p-3uiv #:vertex-attrib-p-3ui
             #:vertex-attrib-p-2uiv #:vertex-attrib-p-2ui
             #:vertex-attrib-p-1uiv #:vertex-attrib-p-1ui
             #:secondary-color-p-3uiv #:secondary-color-p-3ui
             #:color-p-4uiv #:color-p-4ui #:color-p-3uiv
             #:color-p-3ui #:normal-p-3uiv #:normal-p-3ui
             #:multi-tex-coord-p-4uiv #:multi-tex-coord-p-4ui
             #:multi-tex-coord-p-3uiv #:multi-tex-coord-p-3ui
             #:multi-tex-coord-p-2uiv #:multi-tex-coord-p-2ui
             #:multi-tex-coord-p-1uiv #:multi-tex-coord-p-1ui
             #:tex-coord-p-4uiv #:tex-coord-p-4ui #:tex-coord-p-3uiv
             #:tex-coord-p-3ui #:tex-coord-p-2uiv #:tex-coord-p-2ui
             #:tex-coord-p-1uiv #:tex-coord-p-1ui #:vertex-p-4uiv
             #:vertex-p-4ui #:vertex-p-3uiv #:vertex-p-3ui
             #:vertex-p-2uiv #:vertex-p-2ui
             #:load-arb_vertex_type_2_10_10_10_rev
             #:+draw-indirect-buffer+
             #:+draw-indirect-buffer-binding+
             #:draw-elements-indirect #:draw-arrays-indirect
             #:load-arb_draw_indirect #:get-uniform-dv
             #:uniform-matrix-4x-3dv #:uniform-matrix-4x-2dv
             #:uniform-matrix-3x-4dv #:uniform-matrix-3x-2dv
             #:uniform-matrix-2x-4dv #:uniform-matrix-2x-3dv
             #:uniform-matrix-4dv #:uniform-matrix-3dv
             #:uniform-matrix-2dv #:uniform-4dv #:uniform-3dv
             #:uniform-2dv #:uniform-1dv #:uniform-4d #:uniform-3d
             #:uniform-2d #:uniform-1d #:load-arb_gpu_shader_fp64
             ...
             #:+uniform-size+ #:+uniform-name-length+
             #:get-program-stage-iv #:get-uniform-subroutine-uiv
             #:uniform-subroutines-uiv #:get-active-subroutine-name
             #:get-active-subroutine-uniform-name
             #:get-active-subroutine-uniform-iv
             #:get-subroutine-index #:get-subroutine-uniform-location
             #:load-arb_shader_subroutine #:+patch-vertices+
             #:+patch-default-inner-level+
             #:+patch-default-outer-level+
             #:+tess-control-output-vertices+ #:+tess-gen-mode+
             #:+tess-gen-spacing+ #:+tess-gen-vertex-order+
             #:+tess-gen-point-mode+ #:+isolines+ #:+fractional-odd+
             #:+fractional-even+ #:+max-patch-vertices+
             #:+max-tess-gen-level+
             ...
             #:patch-parameter-fv #:patch-parameter-i
             #:load-arb_tessellation_shader #:+transform-feedback+
             #:+transform-feedback-paused+
             #:+transform-feedback-buffer-paused+
             #:+transform-feedback-active+
             #:+transform-feedback-buffer-active+
             #:+transform-feedback-binding+ #:draw-transform-feedback
             #:resume-transform-feedback #:pause-transform-feedback
             #:is-transform-feedback #:gen-transform-feedbacks
             #:delete-transform-feedbacks #:bind-transform-feedback
             #:load-arb_transform_feedback2
             #:+max-transform-feedback-buffers+
             #:+max-vertex-streams+ #:get-query-indexed-iv
             #:end-query-indexed #:begin-query-indexed
             #:draw-transform-feedback-stream
             #:load-arb_transform_feedback3 #:+low-float+
             #:+medium-float+ #:+high-float+ #:+low-int+
             #:+medium-int+ #:+high-int+ #:+shader-compiler+
             #:+shader-binary-formats+ #:+num-shader-binary-formats+
             #:+max-vertex-uniform-vectors+ #:+max-varying-vectors+
             #:+max-fragment-uniform-vectors+ #:+rgb565+
             #:clear-depth-f #:depth-range-f
             #:get-shader-precision-format #:shader-binary
             #:release-shader-compiler #:load-arb_es2_compatibility
             #:+stencil-back-func+ #:+stencil-back-fail+
             #:+stencil-back-pass-depth-fail+
             #:+stencil-back-pass-depth-pass+
             #:+program-binary-length+ #:+num-program-binary-formats+
             #:+program-binary-formats+ #:program-parameter-i
             #:program-binary #:get-program-binary
             #:load-arb_get_program_binary #:+program-separable-ext+
             #:+program-pipeline-binding-ext+ #:+program-separable+
             #:+active-program+ #:+program-pipeline-binding+
             ...
             #:program-uniform-matrix-2fv #:program-uniform-4uiv
             #:program-uniform-4ui #:program-uniform-4dv
             #:program-uniform-4d #:program-uniform-4fv
             #:program-uniform-4f #:program-uniform-4iv
             #:program-uniform-4i #:program-uniform-3uiv
             #:program-uniform-3ui #:program-uniform-3dv
             #:program-uniform-3d #:program-uniform-3fv
             #:program-uniform-3f #:program-uniform-3iv
             #:program-uniform-3i #:program-uniform-2uiv
             #:program-uniform-2ui #:program-uniform-2dv
             #:program-uniform-2d #:program-uniform-2fv
             #:program-uniform-2f #:program-uniform-2iv
             #:program-uniform-2i #:program-uniform-1uiv
             #:program-uniform-1ui #:program-uniform-1dv
             #:program-uniform-1d #:program-uniform-1fv
             #:program-uniform-1f #:program-uniform-1iv
             #:program-uniform-1i #:get-program-pipeline-iv
             #:is-program-pipeline #:gen-program-pipelines
             #:delete-program-pipelines #:bind-program-pipeline
             #:create-shader-programv #:active-shader-program
             #:use-program-stages #:load-arb_separate_shader_objects
             #:+double-mat2+ #:+double-mat3+ #:+double-mat4+
             #:+double-mat2x3+ #:+double-mat2x4+ #:+double-mat3x2+
             #:+double-mat3x4+ #:+double-mat4x2+ #:+double-mat4x3+
             #:get-vertex-attrib-l-dv #:vertex-attrib-lpointer
             #:vertex-attrib-l-4dv #:vertex-attrib-l-3dv
             #:vertex-attrib-l-2dv #:vertex-attrib-l-1dv
             #:vertex-attrib-l-4d #:vertex-attrib-l-3d
             #:vertex-attrib-l-2d #:vertex-attrib-l-1d
             #:load-arb_vertex_attrib_64bit #:+max-viewports+
             #:+viewport-subpixel-bits+ #:+viewport-bounds-range+
             #:+layer-provoking-vertex+
             #:+viewport-index-provoking-vertex+ #:+undefined-vertex+
             #:+first-vertex-convention+ #:+last-vertex-convention+
             #:+provoking-vertex+ #:get-doublei_v #:get-floati_v
             #:depth-range-indexed #:depth-range-arrayv
             #:scissor-indexed-v #:scissor-indexe-d #:scissor-arrayv
             #:viewport-indexed-fv #:viewport-indexed-f
             #:viewport-arrayv #:load-arb_viewport_array
             #:+sync-cl-event-arb+ #:+sync-cl-event-complete-arb+
             #:create-sync-from-clevent-arb #:load-arb_cl_event
             ...
             #:+debug-source-other-arb+ #:+debug-type-error-arb+
             ...
             #:get-debug-message-log-arb #:debug-message-callback-arb
             #:debug-message-insert-arb #:debug-message-control-arb
             #:load-arb_debug_output #:+vertex-shader-bit+
             #:+fragment-shader-bit+ #:+geometry-shader-bit+
             #:+tess-control-shader-bit+
             #:+tess-evaluation-shader-bit+ #:+all-shader-bits+
             ...
             #:+no-reset-notification-arb+ #:getn-uniform-dv-arb
             #:getn-uniform-uiv-arb #:getn-uniform-iv-arb
             #:getn-uniform-fv-arb #:getn-compressed-tex-image-arb
             #:readn-pixels-arb #:getn-tex-image-arb
             #:getn-minmax-arb #:getn-histogram-arb
             #:getn-separable-filter-arb
             #:getn-convolution-filter-arb #:getn-color-table-arb
             #:getn-polygon-stipple-arb #:getn-pixel-map-usv-arb
             #:getn-pixel-map-uiv-arb #:getn-pixel-map-fv-arb
             #:getn-map-iv-arb #:getn-map-fv-arb #:getn-map-dv-arb
             #:get-graphics-reset-status-arb #:load-arb_robustness
             ...
             #:load-arb_base_instance
             #:draw-transform-feedback-stream-instanced
             #:draw-transform-feedback-instanced
             #:load-arb_transform_feedback_instanced
             #:+translated-shader-source-length-angle+
             #:+texture-usage-angle+ #:+framebuffer-attachment-angle+
             #:+pack-reverse-row-order-angle+
             #:+program-binary-angle+ #:get-internalformat-iv
             #:load-arb_internalformat_query
             ...
             #:get-active-atomic-counter-buffer-iv
             #:load-arb_shader_atomic_counters #:+max-image-units+
             #:+image-binding-name+ #:+image-binding-level+
             #:+image-binding-layered+ #:+image-binding-layer+
             #:+image-binding-access+ #:+image-1d+ #:+image-2d+
             #:+image-3d+ #:+image-2d-rect+ #:+image-cube+
             #:+image-buffer+ #:+image-1d-array+ #:+image-2d-array+
             #:+image-cube-map-array+ #:+image-2d-multisample+
             #:+image-2d-multisample-array+ #:+int-image-1d+
             #:+int-image-2d+ #:+int-image-3d+ #:+int-image-2d-rect+
             #:+int-image-cube+ #:+int-image-buffer+
             ...
             #:+max-image-samples+ #:+image-binding-format+
             ...
             #:+max-combined-image-uniforms+ #:memory-barrier
             #:bind-image-texture #:load-arb_shader_image_load_store
             #:+sgx-program-binary-img+ #:+renderbuffer-samples-img+
             #:+framebuffer-incomplete-multisample-img+
             #:+max-samples-img+ #:+texture-samples-img+
             #:+texture-immutable-format+ #:texture-storage-3d-ext
             #:texture-storage-2d-ext #:texture-storage-1d-ext
             #:tex-storage-3d #:tex-storage-2d #:tex-storage-1d
             #:load-arb_texture_storage #:+debug-output-synchronous+
             ...
             #:+debug-source-application+ #:+debug-source-other+
             #:+debug-type-error+ #:+debug-type-deprecated-behavior+
             #:+debug-type-undefined-behavior+
             #:+debug-type-portability+ #:+debug-type-performance+
             #:+debug-type-other+ #:+max-debug-message-length+
             #:+max-debug-logged-messages+ #:+debug-logged-messages+
             #:+debug-severity-high+ #:+debug-severity-medium+
             #:+debug-severity-low+ #:+debug-output+
             #:+context-flag-debug-bit+ #:get-object-ptr-label
             #:object-ptr-label #:get-object-label #:object-label
             #:pop-debug-group #:push-debug-group
             #:get-debug-message-log #:debug-message-callback
             #:debug-message-insert #:debug-message-control
             #:load-khr_debug #:clear-named-buffer-sub-data-ext
             #:clear-named-buffer-data-ext #:clear-buffer-sub-data
             #:clear-buffer-data #:load-arb_clear_buffer_object
             #:+vertex-shader-bit-ext+ #:+fragment-shader-bit-ext+
             #:+all-shader-bits-ext+
             #:+vertex-attrib-array-barrier-bit+
             #:+element-array-barrier-bit+ #:+uniform-barrier-bit+
             #:+texture-fetch-barrier-bit+
             #:+shader-image-access-barrier-bit+
             #:+command-barrier-bit+ #:+pixel-buffer-barrier-bit+
             #:+texture-update-barrier-bit+
             #:+buffer-update-barrier-bit+
             #:+framebuffer-barrier-bit+
             #:+transform-feedback-barrier-bit+
             #:+atomic-counter-barrier-bit+ #:+all-barrier-bits+
             #:+debug-type-marker+ #:+debug-type-push-group+
             #:+debug-type-pop-group+ #:+debug-severity-notification+
             #:+max-debug-group-stack-depth+
             #:+debug-group-stack-depth+ #:+color-attachment-ext+
             #:+multiview-ext+ #:+max-multiview-buffers-ext+
             #:+shader-binary-dmp+ #:+gccso-shader-binary-fj+
             ...
             #:+compute-shader-bit+ #:dispatch-compute-indirect
             #:dispatch-compute #:load-arb_compute_shader
             #:copy-image-sub-data #:load-arb_copy_image #:+buffer+
             #:+shader+ #:+program+ #:+query+ #:+program-pipeline+
             #:+sampler+ #:+display-list+ #:+max-label-length+
             #:+texture-view-min-level+ #:+texture-view-num-levels+
             #:+texture-view-min-layer+ #:+texture-view-num-layers+
             #:+texture-immutable-levels+ #:texture-view
             #:load-arb_texture_view #:+vertex-attrib-binding+
             ...
             #:vertex-binding-divisor #:vertex-attrib-binding
             #:vertex-attrib-lformat #:vertex-attrib-iformat
             #:vertex-attrib-format #:bind-vertex-buffer
             #:load-arb_vertex_attrib_binding
             ...
             #:get-named-framebuffer-parameter-iv-ext
             #:named-framebuffer-parameter-i-ext
             #:get-framebuffer-parameter-iv #:framebuffer-parameter-i
             #:load-arb_framebuffer_no_attachments
             #:+image-format-compatibility-type+
             #:+num-sample-counts+ #:+renderbuffer+
             #:+texture-1d-array+ #:+texture-2d-array+
             #:+texture-cube-map-array+ #:+texture-rectangle+
             #:+texture-buffer+ #:+texture-2d-multisample+
             #:+texture-2d-multisample-array+ #:+texture-compressed+
             #:+internalformat-supported+
             #:+internalformat-preferred+ #:+internalformat-red-size+
             ...
             #:+internalformat-stencil-type+ #:+max-width+
             #:+max-height+ #:+max-depth+ #:+max-layers+
             #:+max-combined-dimensions+ #:+color-components+
             #:+depth-components+ #:+stencil-components+
             #:+color-renderable+ #:+depth-renderable+
             #:+stencil-renderable+ #:+framebuffer-renderable+
             #:+framebuffer-renderable-layered+ #:+framebuffer-blend+
             #:+read-pixels+ #:+read-pixels-format+
             #:+read-pixels-type+ #:+texture-image-format+
             #:+texture-image-type+ #:+get-texture-image-format+
             #:+get-texture-image-type+ #:+mipmap+
             #:+manual-generate-mipmap+ #:+auto-generate-mipmap+
             #:+color-encoding+ #:+srgb-read+ #:+srgb-write+
             #:+srgb-decode-arb+ #:+filter+ #:+vertex-texture+
             #:+tess-control-texture+ #:+tess-evaluation-texture+
             #:+geometry-texture+ #:+fragment-texture+
             #:+compute-texture+ #:+texture-shadow+
             #:+texture-gather+ #:+texture-gather-shadow+
             #:+shader-image-load+ #:+shader-image-store+
             ...
             #:+texture-compressed-block-size+ #:+clear-buffer+
             #:+texture-view+ #:+view-compatibility-class+
             #:+full-support+ #:+caveat-support+
             #:+image-class-4-x-32+ #:+image-class-2-x-32+
             #:+image-class-1-x-32+ #:+image-class-4-x-16+
             #:+image-class-2-x-16+ #:+image-class-1-x-16+
             #:+image-class-4-x-8+ #:+image-class-2-x-8+
             #:+image-class-1-x-8+ #:+image-class-11-11-10+
             #:+image-class-10-10-10-2+ #:+view-class-128-bits+
             #:+view-class-96-bits+ #:+view-class-64-bits+
             #:+view-class-48-bits+ #:+view-class-32-bits+
             #:+view-class-24-bits+ #:+view-class-16-bits+
             ...
             #:+view-class-bptc-float+ #:get-internalformati-64v
             #:load-arb_internalformat_query2
             #:invalidate-sub-framebuffer #:invalidate-framebuffer
             #:invalidate-buffer-data #:invalidate-buffer-sub-data
             #:invalidate-tex-image #:invalidate-tex-sub-image
             #:load-arb_invalidate_subdata
             #:multi-draw-elements-indirect
             #:multi-draw-arrays-indirect
             #:load-arb_multi_draw_indirect #:+uniform+
             #:+uniform-block+ #:+program-input+ #:+program-output+
             #:+buffer-variable+ #:+shader-storage-block+
             #:+atomic-counter-buffer+ #:+vertex-subroutine+
             ...
             #:+transform-feedback-varying+ #:+active-resources+
             #:+max-name-length+ #:+max-num-active-variables+
             #:+max-num-compatible-subroutines+ #:+name-length+
             #:+type+ #:+array-size+ #:+offset+ #:+block-index+
             #:+array-stride+ #:+matrix-stride+ #:+is-row-major+
             #:+atomic-counter-buffer-index+ #:+buffer-binding+
             #:+buffer-data-size+ #:+num-active-variables+
             ...
             #:+location+ #:+location-index+ #:+is-per-patch+
             #:+num-compatible-subroutines+
             #:+compatible-subroutines+
             #:get-program-resource-location-index
             #:get-program-resource-location
             #:get-program-resource-iv #:get-program-resource-name
             #:get-program-resource-index #:get-program-interface-iv
             #:load-arb_program_interface_query
             ...
             #:load-arb_shader_storage_buffer_object
             #:+texture-buffer-offset+ #:+texture-buffer-size+
             #:+texture-buffer-offset-alignment+
             #:texture-buffer-range-ext #:tex-buffer-range
             #:load-arb_texture_buffer_range
             #:texture-storage-3dmultisample-ext
             #:texture-storage-2dmultisample-ext
             #:tex-storage-3dmultisample #:tex-storage-2dmultisample
             #:load-arb_texture_storage_multisample #:blend-color-ext
             #:load-ext_blend_color #:+rescale-normal+
             #:+polygon-offset-ext+ #:+polygon-offset-factor-ext+
             #:polygon-offset-ext #:load-ext_polygon_offset
             #:+texture-3d-binding-oes+ #:+texture-3d-oes+
             #:+texture-wrap-r-oes+ #:+max-3d-texture-size-oes+
             #:tex-sub-image-3d-ext #:tex-image-3d-ext
             #:load-ext_texture3d #:tex-filter-func-sgis
             #:get-tex-filter-func-sgis #:load-sgis_texture_filter4
             #:tex-sub-image-2d-ext #:tex-sub-image-1d-ext
             #:load-ext_subtexture #:copy-tex-sub-image-3d-ext
             #:copy-tex-sub-image-2d-ext #:copy-tex-sub-image-1d-ext
             #:copy-tex-image-2d-ext #:copy-tex-image-1d-ext
             #:load-ext_copy_texture #:+histogram-luminance-size+
             ...
             #:+unsigned-int-2-10-10-10-rev+ #:reset-minmax-ext
             ...
             #:load-ext_histogram #:+histogram+ #:+proxy-histogram+
             #:+histogram-width+ #:+histogram-format+
             #:+histogram-red-size+ #:+histogram-green-size+
             #:+histogram-blue-size+ #:+histogram-alpha-size+
             #:+histogram-sink+ #:+minmax+ #:+minmax-format+
             #:+minmax-sink+ #:+table-too-large+
             ...
             #:load-sgi_color_table #:+texture-min-lod+
             #:+texture-max-lod+ #:+texture-base-level+
             #:+texture-max-level+ #:pixel-tex-gen-sgix
             #:load-sgix_pixel_texture #:+pixel-group-color-sgis+
             ...
             #:load-sgis_pixel_texture #:tex-sub-image-4d-sgis
             #:tex-image-4d-sgis #:load-sgis_texture4d
             #:+pack-skip-images+ #:+pack-image-height+
             #:+unpack-skip-images+ #:+unpack-image-height+
             #:+texture-3d+ #:+proxy-texture-3d+ #:+texture-depth+
             #:+texture-wrap-r+ #:+max-3d-texture-size+
             #:+texture-priority-ext+ #:+texture-resident-ext+
             #:+texture-1d-binding-ext+ #:+texture-2d-binding-ext+
             #:prioritize-textures-ext #:is-texture-ext
             #:gen-textures-ext #:delete-textures-ext
             #:bind-texture-ext #:are-textures-resident-ext
             #:load-ext_texture_object #:+multisample+
             ...
             #:load-sgis_detail_texture #:+color-matrix+
             ...
             #:load-sgis_sharpen_texture #:sample-pattern-sgis
             #:sample-mask-sgis #:load-sgis_multisample
             #:+vertex-array-ext+ #:+normal-array-ext+
             #:+color-array-ext+ #:+index-array-ext+
             #:+texture-coord-array-ext+ #:+edge-flag-array-ext+
             #:+vertex-array-size-ext+ #:+vertex-array-type-ext+
             #:+vertex-array-stride-ext+ #:+normal-array-type-ext+
             #:+normal-array-stride-ext+ #:+color-array-size-ext+
             #:+color-array-type-ext+ #:+color-array-stride-ext+
             ...
             #:tex-coord-pointer-ext #:normal-pointer-ext
             #:index-pointer-ext #:get-pointerv-ext
             #:edge-flag-pointer-ext #:draw-arrays-ext
             #:color-pointer-ext #:array-element-ext
             #:load-ext_vertex_array #:+func-add+ #:+min+ #:+max+
             #:+blend-equation+ #:+blend-equation-rgb+
             #:+blend-equation-rgb-oes+ #:blend-equation-ext
             #:load-ext_blend_minmax #:+constant-border+
             ...
             #:sprite-parameter-f-sgix #:load-sgix_sprite
             #:+point-size-min-ext+ #:+point-size-max-ext+
             #:+point-fade-threshold-size-ext+
             #:+distance-attenuation-ext+ #:point-parameter-fv-ext
             #:point-parameter-f-ext #:load-ext_point_parameters
             #:point-parameter-fv-sgis #:point-parameter-f-sgis
             #:load-sgis_point_parameters #:stop-instruments-sgix
             #:start-instruments-sgix #:read-instruments-sgix
             #:poll-instruments-sgix #:instruments-buffer-sgix
             #:get-instruments-sgix #:load-sgix_instruments
             #:frame-zoom-sgix #:load-sgix_framezoom
             #:tag-sample-buffer-sgix #:load-sgix_tag_sample_buffer
             #:+max-deformation-order-sgix+
             #:load-identity-deformation-map-sgix #:deform-sgix
             #:deformation-map-3f-sgix #:deformation-map-3d-sgix
             #:load-sgix_polynomial_ffd #:reference-plane-sgix
             #:load-sgix_reference_plane #:flush-raster-sgix
             #:load-sgix_flush_raster #:+clamp-to-border+
             #:get-fog-func-sgis #:fog-func-sgis
             #:load-sgis_fog_function #:+image-scale-x-hp+
             #:+image-scale-y-hp+ #:+image-translate-x-hp+
             #:+image-translate-y-hp+ #:+image-rotate-angle-hp+
             #:+image-rotate-origin-x-hp+
             #:+image-rotate-origin-y-hp+ #:+image-mag-filter-hp+
             #:+image-min-filter-hp+ #:+image-cubic-weight-hp+
             #:+cubic-hp+ #:+average-hp+ #:+image-transform-2d-hp+
             ...
             #:load-hp_image_transform #:copy-color-sub-table-ext
             #:color-sub-table-ext #:load-ext_color_subtable
             ...
             #:+always-fast-hint-pgi+ #:+always-soft-hint-pgi+
             #:+allow-draw-obj-hint-pgi+ #:+allow-draw-win-hint-pgi+
             #:+allow-draw-frg-hint-pgi+ #:+allow-draw-mem-hint-pgi+
             ...
             #:+back-normals-hint-pgi+ #:hint-pgi
             #:load-pgi_misc_hints #:+color-index1-ext+
             #:+color-index2-ext+ #:+color-index4-ext+
             #:+color-index8-ext+ #:+color-index12-ext+
             ...
             #:+max-elements-indices-ext+ #:draw-range-elements-ext
             #:load-ext_draw_range_elements #:texture-material-ext
             #:texture-light-ext #:apply-texture-ext
             #:load-ext_light_texture #:is-async-marker-sgix
             #:delete-async-markers-sgix #:gen-async-markers-sgix
             #:poll-async-sgix #:finish-async-sgix
             #:async-marker-sgix #:load-sgix_async
             ...
             #:load-intel_parallel_arrays #:+fragment-material-ext+
             #:+fragment-normal-ext+ #:+fragment-color-ext+
             #:+attenuation-ext+ #:+shadow-attenuation-ext+
             ...
             #:load-ext_pixel_transform #:+color-sum-ext+
             ...
             #:secondary-color-pointer-ext #:secondary-color-3usv-ext
             #:secondary-color-3us-ext #:secondary-color-3uiv-ext
             #:secondary-color-3ui-ext #:secondary-color-3ubv-ext
             #:secondary-color-3ub-ext #:secondary-color-3sv-ext
             #:secondary-color-3s-ext #:secondary-color-3iv-ext
             #:secondary-color-3i-ext #:secondary-color-3fv-ext
             #:secondary-color-3f-ext #:secondary-color-3dv-ext
             #:secondary-color-3d-ext #:secondary-color-3bv-ext
             #:secondary-color-3b-ext #:load-ext_secondary_color
             #:+perturb-ext+ #:+texture-normal-ext+
             #:texture-normal-ext #:load-ext_texture_perturb_normal
             #:multi-draw-elements-ext #:multi-draw-arrays-ext
             #:load-ext_multi_draw_arrays #:+color-sum+
             ...
             #:+secondary-color-array+ #:+fog-coordinate-source-ext+
             #:+fog-coordinate-ext+ #:+fragment-depth-ext+
             ...
             #:+map2-tangent-ext+ #:+map1-binormal-ext+
             #:+map2-binormal-ext+ #:binormal-pointer-ext
             #:tangent-pointer-ext #:binormal-3sv-ext
             ...
             #:tangent-3b-ext #:load-ext_coordinate_frame
             #:+unpack-constant-data-sunx+
             #:+texture-constant-data-sunx+ #:finish-texture-sunx
             #:load-sunx_constant_data #:+global-alpha-sun+
             ...
             #:load-sun_global_alpha #:+restart-sun+
             #:+replace-middle-sun+ #:+replace-oldest-sun+
             ...
             #:+r1ui-c4ub-v3f-sun+ #:+r1ui-c3f-v3f-sun+
             #:+r1ui-n3f-v3f-sun+ #:+r1ui-c4f-n3f-v3f-sun+
             #:+r1ui-t2f-v3f-sun+ #:+r1ui-t2f-n3f-v3f-sun+
             #:+r1ui-t2f-c4f-n3f-v3f-sun+
             #:replacement-code-pointer-sun
             #:replacement-code-ubv-sun #:replacement-code-usv-sun
             #:replacement-code-uiv-sun #:replacement-code-ub-sun
             #:replacement-code-us-sun #:replacement-code-ui-sun
             ...
             #:normal-3f-vertex-3fv-sun #:normal-3f-vertex-3f-sun
             #:color-3f-vertex-3fv-sun #:color-3f-vertex-3f-sun
             #:color-4ub-vertex-3fv-sun #:color-4ub-vertex-3f-sun
             #:color-4ub-vertex-2fv-sun #:color-4ub-vertex-2f-sun
             #:load-sun_vertex #:+blend-dst-rgb-oes+
             #:+blend-src-rgb-oes+ #:+blend-dst-alpha-oes+
             #:+blend-src-alpha-oes+ #:+blend-dst-rgb-ext+
             ...
             #:vertex-weight-pointer-ext #:vertex-weight-fv-ext
             #:vertex-weight-f-ext #:load-ext_vertex_weighting
             ...
             #:vertex-array-range-nv #:flush-vertex-array-range-nv
             #:load-nv_vertex_array_range
             #:+vertex-array-range-without-flush-nv+
             #:+multisample-filter-hint-nv+ #:+register-combiners-nv+
             ...
             #:final-combiner-input-nv #:combiner-output-nv
             #:combiner-input-nv #:combiner-parameter-i-nv
             #:combiner-parameter-iv-nv #:combiner-parameter-f-nv
             #:combiner-parameter-fv-nv #:load-nv_register_combiners
             #:resize-buffers-mesa #:load-mesa_resize_buffers
             #:window-pos-4sv-mesa #:window-pos-4s-mesa
             #:window-pos-4iv-mesa #:window-pos-4i-mesa
             #:window-pos-4fv-mesa #:window-pos-4f-mesa
             #:window-pos-4dv-mesa #:window-pos-4d-mesa
             #:window-pos-3sv-mesa #:window-pos-3s-mesa
             #:window-pos-3iv-mesa #:window-pos-3i-mesa
             #:window-pos-3fv-mesa #:window-pos-3f-mesa
             #:window-pos-3dv-mesa #:window-pos-3d-mesa
             #:window-pos-2sv-mesa #:window-pos-2s-mesa
             #:window-pos-2iv-mesa #:window-pos-2i-mesa
             #:window-pos-2fv-mesa #:window-pos-2f-mesa
             #:window-pos-2dv-mesa #:window-pos-2d-mesa
             ...
             #:+eval-vertex-attrib0-nv+ #:+eval-vertex-attrib1-nv+
             #:+eval-vertex-attrib2-nv+ #:+eval-vertex-attrib3-nv+
             #:+eval-vertex-attrib4-nv+ #:+eval-vertex-attrib5-nv+
             #:+eval-vertex-attrib6-nv+ #:+eval-vertex-attrib7-nv+
             #:+eval-vertex-attrib8-nv+ #:+eval-vertex-attrib9-nv+
             #:+eval-vertex-attrib10-nv+ #:+eval-vertex-attrib11-nv+
             #:+eval-vertex-attrib12-nv+ #:+eval-vertex-attrib13-nv+
             #:+eval-vertex-attrib14-nv+ #:+eval-vertex-attrib15-nv+
             ...
             #:load-nv_evaluators #:+unsigned-identity-nv+
             ...
             #:load-nv_register_combiners2 #:+vertex-program-nv+
             #:+vertex-state-program-nv+ #:+attrib-array-size-nv+
             #:+attrib-array-stride-nv+ #:+attrib-array-type-nv+
             #:+current-attrib-nv+ #:+program-length-nv+
             #:+program-string-nv+ #:+modelview-projection-nv+
             #:+identity-nv+ #:+inverse-nv+ #:+transpose-nv+
             #:+inverse-transpose-nv+
             #:+max-track-matrix-stack-depth-nv+
             #:+max-track-matrices-nv+ #:+matrix0-nv+ #:+matrix1-nv+
             #:+matrix2-nv+ #:+matrix3-nv+ #:+matrix4-nv+
             #:+matrix5-nv+ #:+matrix6-nv+ #:+matrix7-nv+
             #:+current-matrix-stack-depth-nv+ #:+current-matrix-nv+
             #:+vertex-program-point-size-nv+
             #:+vertex-program-two-side-nv+ #:+program-parameter-nv+
             #:+attrib-array-pointer-nv+ #:+program-target-nv+
             #:+program-resident-nv+ #:+track-matrix-nv+
             #:+track-matrix-transform-nv+
             #:+vertex-program-binding-nv+
             #:+program-error-position-nv+
             #:+vertex-attrib-array0-nv+ #:+vertex-attrib-array1-nv+
             #:+vertex-attrib-array2-nv+ #:+vertex-attrib-array3-nv+
             #:+vertex-attrib-array4-nv+ #:+vertex-attrib-array5-nv+
             #:+vertex-attrib-array6-nv+ #:+vertex-attrib-array7-nv+
             #:+vertex-attrib-array8-nv+ #:+vertex-attrib-array9-nv+
             ...
             #:program-parameters-4fv-nv #:program-parameters-4dv-nv
             #:program-parameter-4fv-nv #:program-parameter-4f-nv
             #:program-parameter-4dv-nv #:program-parameter-4d-nv
             #:load-program-nv #:is-program-nv
             #:get-vertex-attrib-pointerv-nv
             #:get-vertex-attrib-iv-nv #:get-vertex-attrib-fv-nv
             #:get-vertex-attrib-dv-nv #:get-track-matrix-iv-nv
             #:get-program-string-nv #:get-program-iv-nv
             #:get-program-parameter-fv-nv
             #:get-program-parameter-dv-nv #:gen-programs-nv
             #:execute-program-nv #:delete-programs-nv
             #:bind-program-nv #:are-programs-resident-nv
             #:load-nv_vertex_program #:+bump-rot-matrix-ati+
             ...
             #:load-ati_envmap_bumpmap #:+fragment-shader-ati+
             #:+reg-0-ati+ #:+reg-1-ati+ #:+reg-2-ati+ #:+reg-3-ati+
             #:+reg-4-ati+ #:+reg-5-ati+ #:+reg-6-ati+ #:+reg-7-ati+
             #:+reg-8-ati+ #:+reg-9-ati+ #:+reg-10-ati+
             #:+reg-11-ati+ #:+reg-12-ati+ #:+reg-13-ati+
             #:+reg-14-ati+ #:+reg-15-ati+ #:+reg-16-ati+
             #:+reg-17-ati+ #:+reg-18-ati+ #:+reg-19-ati+
             #:+reg-20-ati+ #:+reg-21-ati+ #:+reg-22-ati+
             #:+reg-23-ati+ #:+reg-24-ati+ #:+reg-25-ati+
             #:+reg-26-ati+ #:+reg-27-ati+ #:+reg-28-ati+
             #:+reg-29-ati+ #:+reg-30-ati+ #:+reg-31-ati+
             #:+con-0-ati+ #:+con-1-ati+ #:+con-2-ati+ #:+con-3-ati+
             #:+con-4-ati+ #:+con-5-ati+ #:+con-6-ati+ #:+con-7-ati+
             #:+con-8-ati+ #:+con-9-ati+ #:+con-10-ati+
             #:+con-11-ati+ #:+con-12-ati+ #:+con-13-ati+
             #:+con-14-ati+ #:+con-15-ati+ #:+con-16-ati+
             #:+con-17-ati+ #:+con-18-ati+ #:+con-19-ati+
             #:+con-20-ati+ #:+con-21-ati+ #:+con-22-ati+
             #:+con-23-ati+ #:+con-24-ati+ #:+con-25-ati+
             #:+con-26-ati+ #:+con-27-ati+ #:+con-28-ati+
             #:+con-29-ati+ #:+con-30-ati+ #:+con-31-ati+ #:+mov-ati+
             #:+add-ati+ #:+mul-ati+ #:+sub-ati+ #:+dot3-ati+
             #:+dot4-ati+ #:+mad-ati+ #:+lerp-ati+ #:+cnd-ati+
             #:+cnd0-ati+ #:+dot2-add-ati+
             ...
             #:+color-alpha-pairing-ati+ #:+swizzle-str-ati+
             #:+swizzle-stq-ati+ #:+swizzle-str-dr-ati+
             #:+swizzle-stq-dq-ati+ #:+swizzle-strq-ati+
             #:+swizzle-strq-dq-ati+ #:+red-bit-ati+
             #:+green-bit-ati+ #:+blue-bit-ati+ #:+4x-bit-ati+
             #:+8x-bit-ati+ #:+half-bit-ati+ #:+quarter-bit-ati+
             #:+eighth-bit-ati+ #:+saturate-bit-ati+ #:+2x-bit-ati+
             #:+comp-bit-ati+ #:+negate-bit-ati+ #:+bias-bit-ati+
             ...
             #:sample-map-ati #:pass-tex-coord-ati
             #:end-fragment-shader-ati #:begin-fragment-shader-ati
             #:delete-fragment-shader-ati #:bind-fragment-shader-ati
             #:gen-fragment-shaders-ati #:load-ati_fragment_shader
             #:+3dc-x-amd+ #:+3dc-xy-amd+ #:+pn-triangles-ati+
             ...
             #:pntriangles-f-ati #:pntriangles-i-ati
             #:load-ati_pn_triangles #:+buffer-size+ #:+buffer-usage+
             #:+buffer-size-arb+ #:+buffer-usage-arb+ #:+static-ati+
             #:+dynamic-ati+ #:+preserve-ati+ #:+discard-ati+
             ...
             #:variant-array-object-ati #:get-array-object-iv-ati
             #:get-array-object-fv-ati #:array-object-ati
             #:free-object-buffer-ati #:get-object-buffer-iv-ati
             #:get-object-buffer-fv-ati #:update-object-buffer-ati
             #:is-object-buffer-ati #:new-object-buffer-ati
             #:load-ati_vertex_array_object
             #:+atc-rgba-interpolated-alpha-amd+
             #:+vertex-shader-ext+ #:+vertex-shader-binding-ext+
             #:+op-index-ext+ #:+op-negate-ext+ #:+op-dot3-ext+
             #:+op-dot4-ext+ #:+op-mul-ext+ #:+op-add-ext+
             #:+op-madd-ext+ #:+op-frac-ext+ #:+op-max-ext+
             #:+op-min-ext+ #:+op-set-ge-ext+ #:+op-set-lt-ext+
             #:+op-clamp-ext+ #:+op-floor-ext+ #:+op-round-ext+
             #:+op-exp-base-2-ext+ #:+op-log-base-2-ext+
             #:+op-power-ext+ #:+op-recip-ext+ #:+op-recip-sqrt-ext+
             #:+op-sub-ext+ #:+op-cross-product-ext+
             #:+op-multiply-matrix-ext+ #:+op-mov-ext+
             #:+output-vertex-ext+ #:+output-color0-ext+
             #:+output-color1-ext+ #:+output-texture-coord0-ext+
             ...
             #:+output-texture-coord31-ext+ #:+output-fog-ext+
             #:+scalar-ext+ #:+vector-ext+ #:+matrix-ext+
             ...
             #:+vertex-shader-optimized-ext+ #:+x-ext+ #:+y-ext+
             #:+z-ext+ #:+w-ext+ #:+negative-x-ext+
             #:+negative-y-ext+ #:+negative-z-ext+ #:+negative-w-ext+
             #:+zero-ext+ #:+one-ext+ #:+negative-one-ext+
             #:+normalized-range-ext+ #:+full-range-ext+
             #:+current-vertex-ext+ #:+mvp-matrix-ext+
             #:+variant-value-ext+ #:+variant-datatype-ext+
             ...
             #:get-invariant-floatv-ext #:get-invariant-integerv-ext
             #:get-invariant-booleanv-ext #:get-variant-pointerv-ext
             #:get-variant-floatv-ext #:get-variant-integerv-ext
             #:get-variant-booleanv-ext #:is-variant-enabled-ext
             #:bind-parameter-ext #:bind-texture-unit-parameter-ext
             ...
             #:variant-sv-ext #:variant-bv-ext
             #:set-local-constant-ext #:set-invariant-ext
             #:gen-symbols-ext #:extract-component-ext
             ...
             #:begin-vertex-shader-ext #:load-ext_vertex_shader
             ...
             #:load-ati_vertex_streams #:+element-array-ati+
             #:+element-array-type-ati+ #:+element-array-pointer-ati+
             #:draw-range-element-array-ati #:draw-element-array-ati
             #:element-pointer-ati #:load-ati_element_array
             #:+quad-mesh-sun+ #:+triangle-mesh-sun+
             #:draw-mesh-arrays-sun #:load-sun_mesh_array
             #:+current-query-ext+ #:+query-result-ext+
             #:+query-result-available-ext+ #:+pixel-counter-bits-nv+
             ...
             #:gen-occlusion-queries-nv #:load-nv_occlusion_query
             #:+point-sprite-arb+ #:+coord-replace-arb+
             #:+query-counter-bits+ #:+current-query+
             #:+query-result+ #:+query-result-available+
             #:+point-sprite-nv+ #:+coord-replace-nv+
             #:+point-sprite-r-mode-nv+ #:point-parameter-iv-nv
             #:point-parameter-i-nv #:load-nv_point_sprite
             #:+mirror-clamp-to-border-ext+ #:+samples-passed+
             #:+samples-passed-arb+ #:+stencil-test-two-side-ext+
             #:+active-stencil-face-ext+ #:active-stencil-face-ext
             #:load-ext_stencil_two_side #:+element-array-apple+
             ...
             #:load-apple_element_array #:+draw-pixels-apple+
             #:+fence-apple+ #:finish-object-apple
             #:test-object-apple #:finish-fence-apple
             #:test-fence-apple #:is-fence-apple #:set-fence-apple
             #:delete-fences-apple #:gen-fences-apple
             #:load-apple_fence #:+vertex-array-binding-apple+
             ...
             #:vertex-array-range-apple
             #:load-apple_vertex_array_range #:+max-draw-buffers-nv+
             #:+draw-buffer0-nv+ #:+draw-buffer1-nv+
             #:+draw-buffer2-nv+ #:+draw-buffer3-nv+
             #:+draw-buffer4-nv+ #:+draw-buffer5-nv+
             #:+draw-buffer6-nv+ #:+draw-buffer7-nv+
             #:+draw-buffer8-nv+ #:+draw-buffer9-nv+
             #:+draw-buffer10-nv+ #:+draw-buffer11-nv+
             #:+draw-buffer12-nv+ #:+draw-buffer13-nv+
             #:+draw-buffer14-nv+ #:+draw-buffer15-nv+
             #:+color-clear-unclamped-value-ati+
             #:+blend-equation-alpha+ #:+max-draw-buffers-ati+
             #:+draw-buffer0-ati+ #:+draw-buffer1-ati+
             #:+draw-buffer2-ati+ #:+draw-buffer3-ati+
             #:+draw-buffer4-ati+ #:+draw-buffer5-ati+
             #:+draw-buffer6-ati+ #:+draw-buffer7-ati+
             #:+draw-buffer8-ati+ #:+draw-buffer9-ati+
             #:+draw-buffer10-ati+ #:+draw-buffer11-ati+
             #:+draw-buffer12-ati+ #:+draw-buffer13-ati+
             #:+draw-buffer14-ati+ #:+draw-buffer15-ati+
             #:draw-buffers-ati #:load-ati_draw_buffers
             #:+max-vertex-attribs+
             ...
             #:load-nv_fragment_program #:+half-apple+ #:+fixed+
             #:+half-float-nv+ #:vertex-attribs-4hv-nv
             #:vertex-attribs-3hv-nv #:vertex-attribs-2hv-nv
             #:vertex-attribs-1hv-nv #:vertex-attrib-4hv-nv
             #:vertex-attrib-4h-nv #:vertex-attrib-3hv-nv
             #:vertex-attrib-3h-nv #:vertex-attrib-2hv-nv
             #:vertex-attrib-2h-nv #:vertex-attrib-1hv-nv
             #:vertex-attrib-1h-nv #:vertex-weight-hv-nv
             #:vertex-weight-h-nv #:secondary-color-3hv-nv
             #:secondary-color-3h-nv #:fog-coord-hv-nv
             #:fog-coord-h-nv #:multi-tex-coord-4hv-nv
             #:multi-tex-coord-4h-nv #:multi-tex-coord-3hv-nv
             #:multi-tex-coord-3h-nv #:multi-tex-coord-2hv-nv
             #:multi-tex-coord-2h-nv #:multi-tex-coord-1hv-nv
             #:multi-tex-coord-1h-nv #:tex-coord-4hv-nv
             #:tex-coord-4h-nv #:tex-coord-3hv-nv #:tex-coord-3h-nv
             #:tex-coord-2hv-nv #:tex-coord-2h-nv #:tex-coord-1hv-nv
             #:tex-coord-1h-nv #:color-4hv-nv #:color-4h-nv
             #:color-3hv-nv #:color-3h-nv #:normal-3hv-nv
             #:normal-3h-nv #:vertex-4hv-nv #:vertex-4h-nv
             #:vertex-3hv-nv #:vertex-3h-nv #:vertex-2hv-nv
             #:vertex-2h-nv #:load-nv_half_float
             ...
             #:flush-pixel-data-range-nv #:pixel-data-range-nv
             #:load-nv_pixel_data_range #:+primitive-restart-nv+
             #:+primitive-restart-index-nv+
             #:primitive-restart-index-nv #:primitive-restart-nv
             #:load-nv_primitive_restart #:unmap-object-buffer-ati
             #:map-object-buffer-ati #:load-ati_map_object_buffer
             #:+stencil-back-func-ati+ #:+stencil-back-fail-ati+
             #:+stencil-back-pass-depth-fail-ati+
             #:+stencil-back-pass-depth-pass-ati+
             #:stencil-func-separate-ati #:stencil-op-separate-ati
             ...
             #:load-ati_vertex_attrib_array_object #:vertex-4bv-oes
             #:vertex-4b-oes #:vertex-3bv-oes #:vertex-3b-oes
             #:vertex-2bv-oes #:vertex-2b-oes #:tex-coord-4bv-oes
             #:tex-coord-4b-oes #:tex-coord-3bv-oes
             #:tex-coord-3b-oes #:tex-coord-2bv-oes
             #:tex-coord-2b-oes #:tex-coord-1bv-oes
             #:tex-coord-1b-oes #:multi-tex-coord-4bv-oes
             #:multi-tex-coord-4b-oes #:multi-tex-coord-3bv-oes
             #:multi-tex-coord-3b-oes #:multi-tex-coord-2bv-oes
             #:multi-tex-coord-2b-oes #:multi-tex-coord-1bv-oes
             #:multi-tex-coord-1b-oes #:load-oes_byte_coordinates
             #:+fixed-oes+ #:vertex-4xv-oes #:vertex-3xv-oes
             #:vertex-2xv-oes #:vertex-4x-oes #:vertex-3x-oes
             #:vertex-2x-oes #:translatex-oes #:tex-parameterxv-oes
             #:tex-parameterx-oes #:tex-genxv-oes #:tex-genx-oes
             #:tex-envxv-oes #:tex-envx-oes #:tex-coord-4xv-oes
             #:tex-coord-3xv-oes #:tex-coord-2xv-oes
             #:tex-coord-1xv-oes #:tex-coord-4x-oes
             #:tex-coord-3x-oes #:tex-coord-2x-oes #:tex-coord-1x-oes
             #:scalex-oes #:sample-coverage-oes #:rotatex-oes
             #:rectxv-oes #:rectx-oes #:raster-pos-4xv-oes
             #:raster-pos-3xv-oes #:raster-pos-2xv-oes
             #:raster-pos-4x-oes #:raster-pos-3x-oes
             #:raster-pos-2x-oes #:prioritize-texturesx-oes
             #:polygon-offsetx-oes #:point-sizex-oes
             #:point-parameterxv-oes #:pixel-zoomx-oes
             #:pixel-transferx-oes #:pixel-storex #:pixel-mapx
             #:pass-throughx-oes #:orthox-oes #:normal-3xv-oes
             #:normal-3x-oes #:multi-tex-coord-4xv-oes
             #:multi-tex-coord-3xv-oes #:multi-tex-coord-2xv-oes
             #:multi-tex-coord-1xv-oes #:multi-tex-coord-4x-oes
             #:multi-tex-coord-3x-oes #:multi-tex-coord-2x-oes
             #:multi-tex-coord-1x-oes #:mult-transpose-matrixx-oes
             #:mult-matrixx-oes #:materialxv-oes #:materialx-oes
             #:map-grid-2x-oes #:map-grid-1x-oes #:map-2x-oes
             #:map-1x-oes #:load-transpose-matrixx-oes
             #:load-matrixx-oes #:line-widthx-oes #:lightxv-oes
             #:lightx-oes #:light-modelxv-oes #:light-modelx-oes
             #:indexxv-oes #:indexx-oes #:get-tex-parameterxv-oes
             #:get-tex-level-parameterxv-oes #:get-tex-genxv-oes
             #:get-tex-envxv-oes #:get-pixel-mapxv
             #:get-materialx-oes #:get-mapxv-oes #:get-lightx-oes
             #:get-histogram-parameterxv-oes #:get-fixedv-oes
             #:get-convolution-parameterxv-oes #:get-clip-planex-oes
             #:frustumx-oes #:fogxv-oes #:fogx-oes
             #:feedback-bufferx-oes #:eval-coord-2xv-oes
             #:eval-coord-1xv-oes #:eval-coord-2x-oes
             #:eval-coord-1x-oes #:depth-rangex-oes
             #:convolution-parameterxv-oes
             #:convolution-parameterx-oes #:color-4xv-oes
             #:color-3xv-oes #:color-4x-oes #:color-3x-oes
             #:clip-planex-oes #:clear-depthx-oes #:clear-colorx-oes
             #:clear-accumx-oes #:blend-colorx-oes #:bitmapx-oes
             #:alpha-funcx-oes #:accumx-oes #:load-oes_fixed_point
             #:get-clip-planef-oes #:clear-depthf-oes
             #:clip-planef-oes #:orthof-oes #:frustumf-oes
             #:depth-rangef-oes #:load-oes_single_precision
             #:query-matrixx-oes #:load-oes_query_matrix
             #:+array-buffer+ #:+element-array-buffer+
             ...
             #:depth-bounds-ext #:load-ext_depth_bounds_test
             #:+blend-equation-alpha-oes+ #:+blend-equation-rgb-ext+
             ...
             #:+max-color-attachments-ext+ #:+color-attachment0-ext+
             #:+color-attachment1-ext+ #:+color-attachment2-ext+
             #:+color-attachment3-ext+ #:+color-attachment4-ext+
             #:+color-attachment5-ext+ #:+color-attachment6-ext+
             #:+color-attachment7-ext+ #:+color-attachment8-ext+
             #:+color-attachment9-ext+ #:+color-attachment10-ext+
             #:+color-attachment11-ext+ #:+color-attachment12-ext+
             #:+color-attachment13-ext+ #:+color-attachment14-ext+
             #:+color-attachment15-ext+ #:+depth-attachment-ext+
             #:+stencil-attachment-ext+ #:+framebuffer-ext+
             #:+renderbuffer-ext+ #:+renderbuffer-width-ext+
             #:+renderbuffer-height-ext+
             #:+renderbuffer-internal-format-ext+
             #:+stencil-index1-ext+ #:+stencil-index4-ext+
             #:+stencil-index8-ext+ #:+stencil-index16-ext+
             ...
             #:is-renderbuffer-ext #:load-ext_framebuffer_object
             #:string-marker-gremedy #:load-gremedy_string_marker
             #:+max-program-exec-instructions-nv+
             #:+max-program-call-depth-nv+ #:+stencil-tag-bits-ext+
             #:+stencil-clear-tag-value-ext+ #:stencil-clear-tag-ext
             #:load-ext_stencil_clear_tag #:+read-framebuffer-nv+
             #:+draw-framebuffer-nv+ #:+draw-framebuffer-binding-nv+
             #:+read-framebuffer-binding-nv+ #:+read-framebuffer-ext+
             #:+draw-framebuffer-ext+
             #:+draw-framebuffer-binding-ext+
             #:+read-framebuffer-binding-ext+ #:blit-framebuffer-ext
             #:load-ext_framebuffer_blit #:+renderbuffer-samples-nv+
             #:+renderbuffer-samples-ext+
             #:+framebuffer-incomplete-multisample-ext+
             #:+max-samples-ext+
             #:renderbuffer-storage-multisample-ext
             #:load-ext_framebuffer_multisample #:+matrix0-arb+
             #:+matrix1-arb+ #:+matrix2-arb+ #:+matrix3-arb+
             #:+matrix4-arb+ #:+matrix5-arb+ #:+matrix6-arb+
             #:+matrix7-arb+ #:+matrix8-arb+ #:+matrix9-arb+
             ...
             #:+matrix31-arb+ #:+stream-draw+ #:+stream-read+
             #:+stream-copy+ #:+static-draw+ #:+static-read+
             #:+static-copy+ #:+dynamic-draw+ #:+dynamic-read+
             #:+dynamic-copy+ #:+stream-draw-arb+ #:+stream-read-arb+
             #:+stream-copy-arb+ #:+static-draw-arb+
             #:+static-read-arb+ #:+static-copy-arb+
             #:+dynamic-draw-arb+ #:+dynamic-read-arb+
             #:+dynamic-copy-arb+ #:+time-elapsed-ext+
             ...
             #:program-local-parameter-i-4i-nv #:load-nv_gpu_program4
             #:+geometry-program-nv+
             ...
             #:+geometry-input-type-arb+ #:+geometry-output-type-arb+
             ...
             #:+program-point-size-ext+ #:program-parameter-i-ext
             ...
             #:vertex-attrib-i-1i-ext #:load-nv_vertex_program4
             #:+sampler-2d-array-shadow-nv+
             #:+sampler-cube-shadow-nv+ #:+sampler-1d-array-ext+
             ...
             #:load-ext_gpu_shader4 #:draw-elements-instanced-ext
             #:draw-arrays-instanced-ext #:load-ext_draw_instanced
             ...
             #:+interleaved-attribs+ #:+separate-attribs+
             #:+transform-feedback-buffer+
             #:+transform-feedback-buffer-binding+ #:+atc-rgb-amd+
             #:+atc-rgba-explicit-alpha-amd+
             #:+back-primary-color-nv+ #:+back-secondary-color-nv+
             #:+texture-coord-nv+ #:+clip-distance-nv+
             #:+vertex-id-nv+ #:+primitive-id-nv+
             ...
             #:+next-buffer-nv+ #:+skip-components4-nv+
             #:+skip-components3-nv+ #:+skip-components2-nv+
             #:+skip-components1-nv+
             #:transform-feedback-stream-attribs-nv
             #:get-transform-feedback-varying-nv
             #:get-active-varying-nv #:get-varying-location-nv
             #:active-varying-nv #:transform-feedback-varyings-nv
             #:bind-buffer-base-nv #:bind-buffer-offset-nv
             #:bind-buffer-range-nv #:transform-feedback-attribs-nv
             ...
             #:load-ext_bindable_uniform #:+rgba32ui+ #:+rgb32ui+
             #:+rgba16ui+ #:+rgb16ui+ #:+rgba8ui+ #:+rgb8ui+
             #:+rgba32i+ #:+rgb32i+ #:+rgba16i+ #:+rgb16i+ #:+rgba8i+
             #:+rgb8i+ #:+red-integer+ #:+green-integer+
             #:+blue-integer+ #:+alpha-integer+ #:+rgb-integer+
             #:+rgba-integer+ #:+bgr-integer+ #:+bgra-integer+
             #:+rgba32ui-ext+ #:+rgb32ui-ext+ #:+alpha32ui-ext+
             #:+intensity32ui-ext+ #:+luminance32ui-ext+
             #:+luminance-alpha32ui-ext+ #:+rgba16ui-ext+
             #:+rgb16ui-ext+ #:+alpha16ui-ext+ #:+intensity16ui-ext+
             #:+luminance16ui-ext+ #:+luminance-alpha16ui-ext+
             #:+rgba8ui-ext+ #:+rgb8ui-ext+ #:+alpha8ui-ext+
             #:+intensity8ui-ext+ #:+luminance8ui-ext+
             #:+luminance-alpha8ui-ext+ #:+rgba32i-ext+
             #:+rgb32i-ext+ #:+alpha32i-ext+ #:+intensity32i-ext+
             #:+luminance32i-ext+ #:+luminance-alpha32i-ext+
             #:+rgba16i-ext+ #:+rgb16i-ext+ #:+alpha16i-ext+
             #:+intensity16i-ext+ #:+luminance16i-ext+
             ...
             #:+green-integer-ext+ #:+blue-integer-ext+
             #:+alpha-integer-ext+ #:+rgb-integer-ext+
             #:+rgba-integer-ext+ #:+bgr-integer-ext+
             #:+bgra-integer-ext+ #:+luminance-integer-ext+
             #:+luminance-alpha-integer-ext+
             #:+rgba-integer-mode-ext+ #:clear-color-i-ui-ext
             #:clear-color-i-i-ext #:get-tex-parameter-i-uiv-ext
             #:get-tex-parameter-i-iv-ext #:tex-parameter-i-uiv-ext
             #:tex-parameter-i-iv-ext #:load-ext_texture_integer
             #:frame-terminator-gremedy
             #:load-gremedy_frame_terminator #:+query-wait-nv+
             #:+query-no-wait-nv+ #:+query-by-region-wait-nv+
             #:+query-by-region-no-wait-nv+
             #:end-conditional-render-nv
             #:begin-conditional-render-nv
             #:load-nv_conditional_render #:+frame-nv+ #:+fields-nv+
             #:+current-time-nv+ #:+num-fill-streams-nv+
             #:+present-time-nv+ #:+present-duration-nv+
             #:get-videoui-64v-nv #:get-videoi-64v-nv
             #:get-video-uiv-nv #:get-video-iv-nv
             #:present-frame-dual-fill-nv #:present-frame-keyed-nv
             ...
             #:+interleaved-attribs-ext+ #:+separate-attribs-ext+
             ...
             #:transform-feedback-varyings-ext #:bind-buffer-base-ext
             #:bind-buffer-offset-ext #:bind-buffer-range-ext
             #:end-transform-feedback-ext
             #:begin-transform-feedback-ext
             #:load-ext_transform_feedback #:+program-matrix-ext+
             ...
             #:program-uniform-4dv-ext #:program-uniform-3dv-ext
             #:program-uniform-2dv-ext #:program-uniform-1dv-ext
             #:program-uniform-4d-ext #:program-uniform-3d-ext
             #:program-uniform-2d-ext #:program-uniform-1d-ext
             #:multi-tex-renderbuffer-ext #:texture-renderbuffer-ext
             ...
             #:named-renderbuffer-storage-ext #:multi-tex-buffer-ext
             #:texture-buffer-ext #:get-named-buffer-sub-data-ext
             ...
             #:map-named-buffer-range-ext #:unmap-named-buffer-ext
             #:map-named-buffer-ext #:named-buffer-sub-data-ext
             #:named-buffer-data-ext #:program-uniform-4uiv-ext
             #:program-uniform-3uiv-ext #:program-uniform-2uiv-ext
             #:program-uniform-1uiv-ext #:program-uniform-4ui-ext
             #:program-uniform-3ui-ext #:program-uniform-2ui-ext
             ...
             #:program-uniform-4iv-ext #:program-uniform-3iv-ext
             #:program-uniform-2iv-ext #:program-uniform-1iv-ext
             #:program-uniform-4fv-ext #:program-uniform-3fv-ext
             #:program-uniform-2fv-ext #:program-uniform-1fv-ext
             #:program-uniform-4i-ext #:program-uniform-3i-ext
             #:program-uniform-2i-ext #:program-uniform-1i-ext
             #:program-uniform-4f-ext #:program-uniform-3f-ext
             #:program-uniform-2f-ext #:program-uniform-1f-ext
             ...
             #:get-pointer-indexed-v-ext #:get-double-indexed-v-ext
             #:get-float-indexed-v-ext #:get-multi-tex-gen-iv-ext
             #:get-multi-tex-gen-fv-ext #:get-multi-tex-gen-dv-ext
             #:get-multi-tex-env-iv-ext #:get-multi-tex-env-fv-ext
             #:multi-tex-gen-iv-ext #:multi-tex-gen-i-ext
             #:multi-tex-gen-fv-ext #:multi-tex-gen-f-ext
             #:multi-tex-gen-dv-ext #:multi-tex-gen-d-ext
             #:multi-tex-env-iv-ext #:multi-tex-env-i-ext
             #:multi-tex-env-fv-ext #:multi-tex-env-f-ext
             ...
             #:multi-tex-sub-image-3d-ext #:multi-tex-image-3d-ext
             ...
             #:multi-tex-sub-image-1d-ext #:multi-tex-image-2d-ext
             #:multi-tex-image-1d-ext #:multi-tex-parameter-iv-ext
             #:multi-tex-parameter-i-ext #:multi-tex-parameter-fv-ext
             #:multi-tex-parameter-f-ext
             #:copy-texture-sub-image-3d-ext
             #:texture-sub-image-3d-ext #:texture-image-3d-ext
             #:get-texture-level-parameter-iv-ext
             #:get-texture-level-parameter-fv-ext
             #:get-texture-parameter-iv-ext
             #:get-texture-parameter-fv-ext #:get-texture-image-ext
             #:copy-texture-sub-image-2d-ext
             #:copy-texture-sub-image-1d-ext
             #:copy-texture-image-2d-ext #:copy-texture-image-1d-ext
             #:texture-sub-image-2d-ext #:texture-sub-image-1d-ext
             #:texture-image-2d-ext #:texture-image-1d-ext
             #:texture-parameter-iv-ext #:texture-parameter-i-ext
             #:texture-parameter-fv-ext #:texture-parameter-f-ext
             #:matrix-mult-transpose-d-ext
             #:matrix-mult-transpose-f-ext
             #:matrix-load-transpose-d-ext
             #:matrix-load-transpose-f-ext #:matrix-push-ext
             #:matrix-pop-ext #:matrix-ortho-ext #:matrix-frustum-ext
             #:matrix-translate-d-ext #:matrix-translate-f-ext
             #:matrix-scale-d-ext #:matrix-scale-f-ext
             #:matrix-rotate-d-ext #:matrix-rotate-f-ext
             #:matrix-load-identity-ext #:matrix-mult-d-ext
             #:matrix-mult-f-ext #:matrix-load-d-ext
             #:matrix-load-f-ext #:push-client-attrib-default-ext
             #:client-attrib-default-ext
             #:load-ext_direct_state_access #:+sample-position-nv+
             #:+sample-mask-nv+ #:+sample-mask-value-nv+
             ...
             #:+max-sample-mask-words-nv+ #:tex-renderbuffer-nv
             #:sample-mask-indexed-nv #:get-multisample-fv-nv
             #:load-nv_explicit_multisample #:+transform-feedback-nv+
             ...
             #:pause-transform-feedback-nv #:is-transform-feedback-nv
             #:gen-transform-feedbacks-nv
             #:delete-transform-feedbacks-nv
             #:bind-transform-feedback-nv
             #:load-nv_transform_feedback2 #:+texture-width-qcom+
             #:+texture-height-qcom+ #:+texture-depth-qcom+
             ...
             #:+modulate-color-img+ #:+recip-add-signed-alpha-img+
             #:+texture-alpha-modulate-img+
             #:+factor-alpha-modulate-img+
             #:+fragment-alpha-modulate-img+ #:+add-blend-img+
             #:+sgx-binary-img+ #:+counter-type-amd+
             #:+counter-range-amd+ #:+unsigned-int64-amd+
             #:+percentage-amd+ #:+perfmon-result-available-amd+
             #:+perfmon-result-size-amd+ #:+perfmon-result-amd+
             #:get-perf-monitor-counter-data-amd
             #:end-perf-monitor-amd #:begin-perf-monitor-amd
             #:select-perf-monitor-counters-amd
             #:delete-perf-monitors-amd #:gen-perf-monitors-amd
             ...
             #:load-amd_performance_monitor #:+sampler-buffer-amd+
             ...
             #:provoking-vertex-ext #:load-ext_provoking_vertex
             #:blend-equation-separate-indexed-amd
             #:blend-equation-indexed-amd
             #:blend-func-separate-indexed-amd
             #:blend-func-indexed-amd #:load-amd_draw_buffers_blend
             ...
             #:get-tex-parameter-pointerv-apple #:texture-range-apple
             #:load-apple_texture_range #:+vertex-attrib-map1-apple+
             ...
             #:+buffer-object-apple+ #:+released-apple+
             #:+volatile-apple+ #:+retained-apple+
             #:+undefined-apple+ #:+purgeable-apple+
             #:get-object-parameter-iv-apple
             #:object-unpurgeable-apple #:object-purgeable-apple
             #:load-apple_object_purgeable #:+video-buffer-nv+
             #:+video-buffer-binding-nv+ #:+field-upper-nv+
             ...
             #:+partial-success-nv+ #:+success-nv+ #:+failure-nv+
             #:+ycbycr8-422-nv+ #:+ycbaycr8a-4224-nv+
             ...
             #:video-capture-nv #:get-video-capture-stream-dv-nv
             #:get-video-capture-stream-fv-nv
             #:get-video-capture-stream-iv-nv
             #:get-video-capture-iv-nv #:end-video-capture-nv
             #:bind-video-capture-stream-texture-nv
             #:bind-video-capture-stream-buffer-nv
             #:begin-video-capture-nv #:load-nv_video_capture
             #:copy-image-sub-data-nv #:load-nv_copy_image
             #:+palette4-rgb8-oes+ #:+palette4-rgba8-oes+
             #:+palette4-r5-g6-b5-oes+ #:+palette4-rgba4-oes+
             #:+palette4-rgb5-a1-oes+ #:+palette8-rgb8-oes+
             ...
             #:+point-size-array-oes+ #:+texture-crop-rect-oes+
             #:+matrix-index-array-buffer-binding-oes+
             #:+point-size-array-buffer-binding-oes+
             #:+active-program-ext+ #:create-shader-program-ext
             #:active-program-ext #:use-shader-program-ext
             #:load-ext_separate_shader_objects
             #:+buffer-gpu-address-nv+ #:program-uniformui-64v-nv
             #:program-uniformui-64-nv #:get-uniformui-64v-nv
             #:uniformui-64v-nv #:uniformui-64-nv
             ...
             #:make-named-buffer-resident-nv #:is-buffer-resident-nv
             #:make-buffer-non-resident-nv #:make-buffer-resident-nv
             #:load-nv_shader_buffer_load #:+gpu-address-nv+
             #:+max-shader-buffer-address-nv+
             #:+vertex-attrib-array-unified-nv+
             #:+element-array-unified-nv+
             #:+vertex-attrib-array-address-nv+
             #:+vertex-array-address-nv+ #:+normal-array-address-nv+
             #:+color-array-address-nv+ #:+index-array-address-nv+
             ...
             #:+vertex-array-length-nv+ #:+normal-array-length-nv+
             #:+color-array-length-nv+ #:+index-array-length-nv+
             #:+texture-coord-array-length-nv+
             #:+edge-flag-array-length-nv+
             #:+secondary-color-array-length-nv+
             #:+fog-coord-array-length-nv+
             #:+element-array-length-nv+ #:+draw-indirect-unified-nv+
             #:+draw-indirect-address-nv+ #:+draw-indirect-length-nv+
             #:get-integerui-64i_v-nv #:vertex-attrib-iformat-nv
             #:vertex-attrib-format-nv #:fog-coord-format-nv
             #:secondary-color-format-nv #:edge-flag-format-nv
             #:tex-coord-format-nv #:index-format-nv
             #:color-format-nv #:normal-format-nv #:vertex-format-nv
             #:buffer-address-range-nv
             #:load-nv_vertex_buffer_unified_memory
             #:texture-barrier-nv #:load-nv_texture_barrier
             #:+max-image-units-ext+
             #:+max-combined-image-units-and-fragment-outputs-ext+
             #:+image-binding-name-ext+ #:+image-binding-level-ext+
             #:+image-binding-layered-ext+
             #:+image-binding-layer-ext+ #:+image-binding-access-ext+
             #:+image-1d-ext+ #:+image-2d-ext+ #:+image-3d-ext+
             #:+image-2d-rect-ext+ #:+image-cube-ext+
             #:+image-buffer-ext+ #:+image-1d-array-ext+
             #:+image-2d-array-ext+ #:+image-cube-map-array-ext+
             #:+image-2d-multisample-ext+
             #:+image-2d-multisample-array-ext+ #:+int-image-1d-ext+
             #:+int-image-2d-ext+ #:+int-image-3d-ext+
             #:+int-image-2d-rect-ext+ #:+int-image-cube-ext+
             #:+int-image-buffer-ext+ #:+int-image-1d-array-ext+
             ...
             #:vertex-attrib-l-3dv-ext #:vertex-attrib-l-2dv-ext
             #:vertex-attrib-l-1dv-ext #:vertex-attrib-l-4d-ext
             #:vertex-attrib-l-3d-ext #:vertex-attrib-l-2d-ext
             #:vertex-attrib-l-1d-ext #:load-ext_vertex_attrib_64bit
             ...
             #:load-nv_gpu_program5 #:+double-vec2+ #:+double-vec3+
             #:+double-vec4+ #:+int8-nv+ #:+int8-vec2-nv+
             #:+int8-vec3-nv+ #:+int8-vec4-nv+ #:+int16-nv+
             #:+int16-vec2-nv+ #:+int16-vec3-nv+ #:+int16-vec4-nv+
             #:+int64-vec2-nv+ #:+int64-vec3-nv+ #:+int64-vec4-nv+
             #:+unsigned-int8-nv+ #:+unsigned-int8-vec2-nv+
             #:+unsigned-int8-vec3-nv+ #:+unsigned-int8-vec4-nv+
             #:+unsigned-int16-nv+ #:+unsigned-int16-vec2-nv+
             #:+unsigned-int16-vec3-nv+ #:+unsigned-int16-vec4-nv+
             #:+unsigned-int64-vec2-nv+ #:+unsigned-int64-vec3-nv+
             #:+unsigned-int64-vec4-nv+ #:+float16-nv+
             #:+float16-vec2-nv+ #:+float16-vec3-nv+
             #:+float16-vec4-nv+ #:+patches+
             #:program-uniform-4ui-64v-nv
             #:program-uniform-3ui-64v-nv
             #:program-uniform-2ui-64v-nv
             #:program-uniform-1ui-64v-nv #:program-uniform-4ui-64-nv
             #:program-uniform-3ui-64-nv #:program-uniform-2ui-64-nv
             #:program-uniform-1ui-64-nv #:program-uniform-4i-64v-nv
             #:program-uniform-3i-64v-nv #:program-uniform-2i-64v-nv
             #:program-uniform-1i-64v-nv #:program-uniform-4i-64-nv
             #:program-uniform-3i-64-nv #:program-uniform-2i-64-nv
             #:program-uniform-1i-64-nv #:get-uniformi-64v-nv
             #:uniform-4ui-64v-nv #:uniform-3ui-64v-nv
             #:uniform-2ui-64v-nv #:uniform-1ui-64v-nv
             #:uniform-4ui-64-nv #:uniform-3ui-64-nv
             #:uniform-2ui-64-nv #:uniform-1ui-64-nv
             #:uniform-4i-64v-nv #:uniform-3i-64v-nv
             #:uniform-2i-64v-nv #:uniform-1i-64v-nv
             #:uniform-4i-64-nv #:uniform-3i-64-nv #:uniform-2i-64-nv
             #:uniform-1i-64-nv #:load-nv_gpu_shader5 #:+int64-nv+
             #:+unsigned-int64-nv+ #:vertex-attrib-lformat-nv
             #:get-vertex-attrib-lui-64v-nv
             #:get-vertex-attrib-li-64v-nv
             #:vertex-attrib-l4ui-64v-nv #:vertex-attrib-l3ui-64v-nv
             #:vertex-attrib-l2ui-64v-nv #:vertex-attrib-l1ui-64v-nv
             #:vertex-attrib-l4ui-64-nv #:vertex-attrib-l3ui-64-nv
             #:vertex-attrib-l2ui-64-nv #:vertex-attrib-l1ui-64-nv
             #:vertex-attrib-l4i-64v-nv #:vertex-attrib-l3i-64v-nv
             #:vertex-attrib-l2i-64v-nv #:vertex-attrib-l1i-64v-nv
             #:vertex-attrib-l4i-64-nv #:vertex-attrib-l3i-64-nv
             #:vertex-attrib-l2i-64-nv #:vertex-attrib-l1i-64-nv
             #:load-nv_vertex_attrib_integer_64bit
             #:+buffer-object-ext+ #:+query-object-ext+
             #:+vertex-array-object-ext+ #:+data-buffer-amd+
             #:+performance-monitor-amd+ #:+query-object-amd+
             #:+vertex-array-object-amd+ #:+sampler-object-amd+
             #:is-name-amd #:delete-names-amd #:gen-names-amd
             ...
             #:+debug-category-other-amd+ #:get-debug-message-log-amd
             #:debug-message-callback-amd #:debug-message-insert-amd
             #:debug-message-enable-amd #:load-amd_debug_output
             #:+dot-product-texture-3d-nv+ #:+surface-state-nv+
             #:+surface-registered-nv+ #:+surface-mapped-nv+
             #:+write-discard-nv+ #:vdpauunmap-surfaces-nv
             #:vdpaumap-surfaces-nv #:vdpausurface-access-nv
             #:vdpauget-surface-iv-nv #:vdpauunregister-surface-nv
             #:vdpauis-surface-nv #:vdpauregister-output-surface-nv
             #:vdpauregister-video-surface-nv #:vdpaufin-i-nv
             #:vdpauinit-nv #:load-nv_vdpau_interop
             ...
             #:load-nv_texture_multisample #:+subsample-distance-amd+
             #:set-multisamplefv-amd #:load-amd_sample_positions
             #:+sync-x11-fence-ext+ #:import-sync-ext
             #:load-ext_x11_sync_object
             #:multi-draw-elements-indirect-amd
             #:multi-draw-arrays-indirect-amd
             #:load-amd_multi_draw_indirect #:+path-format-svg-nv+
             #:+path-format-ps-nv+ #:+standard-font-name-nv+
             #:+system-font-name-nv+ #:+file-name-nv+
             #:+path-stroke-width-nv+ #:+path-end-caps-nv+
             #:+path-initial-end-cap-nv+ #:+path-terminal-end-cap-nv+
             #:+path-join-style-nv+ #:+path-miter-limit-nv+
             #:+path-dash-caps-nv+ #:+path-initial-dash-cap-nv+
             #:+path-terminal-dash-cap-nv+ #:+path-dash-offset-nv+
             #:+path-client-length-nv+ #:+path-fill-mode-nv+
             #:+path-fill-mask-nv+ #:+path-fill-cover-mode-nv+
             #:+path-stroke-cover-mode-nv+ #:+path-stroke-mask-nv+
             #:+count-up-nv+ #:+count-down-nv+
             #:+path-object-bounding-box-nv+ #:+convex-hull-nv+
             #:+bounding-box-nv+ #:+translate-x-nv+
             #:+translate-y-nv+ #:+translate-2d-nv+
             #:+translate-3d-nv+ #:+affine-2d-nv+ #:+affine-3d-nv+
             #:+transpose-affine-2d-nv+ #:+transpose-affine-3d-nv+
             #:+utf8-nv+ #:+utf16-nv+
             ...
             #:+path-stroke-bounding-box-nv+ #:+square-nv+
             #:+round-nv+ #:+triangular-nv+ #:+bevel-nv+
             #:+miter-revert-nv+ #:+miter-truncate-nv+
             #:+skip-missing-glyph-nv+ #:+use-missing-glyph-nv+
             #:+path-error-position-nv+ #:+path-fog-gen-mode-nv+
             #:+accum-adjacent-pairs-nv+ #:+adjacent-pairs-nv+
             #:+first-to-rest-nv+ #:+path-gen-mode-nv+
             #:+path-gen-coeff-nv+ #:+path-gen-color-format-nv+
             #:+path-gen-components-nv+ #:+path-stencil-func-nv+
             #:+path-stencil-ref-nv+ #:+path-stencil-value-mask-nv+
             #:+path-stencil-depth-offset-factor-nv+
             #:+path-stencil-depth-offset-units-nv+
             #:+path-cover-depth-func-nv+
             #:+path-dash-offset-reset-nv+ #:+move-to-resets-nv+
             #:+move-to-continues-nv+ #:+close-path-nv+
             #:+move-to-nv+ #:+relative-move-to-nv+ #:+line-to-nv+
             #:+relative-line-to-nv+ #:+horizontal-line-to-nv+
             #:+relative-horizontal-line-to-nv+
             #:+vertical-line-to-nv+ #:+relative-vertical-line-to-nv+
             #:+quadratic-curve-to-nv+
             #:+relative-quadratic-curve-to-nv+ #:+cubic-curve-to-nv+
             ...
             #:+dup-last-cubic-curve-to-nv+ #:+rect-nv+
             #:+circular-ccw-arc-to-nv+ #:+circular-cw-arc-to-nv+
             #:+circular-tangent-arc-to-nv+ #:+arc-to-nv+
             ...
             #:+font-units-per-em-bit-nv+ #:+font-ascender-bit-nv+
             #:+font-descender-bit-nv+ #:+font-height-bit-nv+
             #:+font-max-advance-width-bit-nv+
             #:+font-max-advance-height-bit-nv+
             #:+font-underline-position-bit-nv+
             #:+font-underline-thickness-bit-nv+
             #:+font-has-kerning-bit-nv+ #:+primary-color+
             #:+primary-color-nv+ #:+secondary-color-nv+
             #:point-along-path-nv #:get-path-length-nv
             #:is-point-in-stroke-path-nv #:is-point-in-fill-path-nv
             #:get-path-tex-gen-fv-nv #:get-path-tex-gen-iv-nv
             #:get-path-color-gen-fv-nv #:get-path-color-gen-iv-nv
             #:get-path-spacing-nv #:get-path-metric-range-nv
             #:get-path-metrics-nv #:get-path-dash-array-nv
             #:get-path-coords-nv #:get-path-commands-nv
             #:get-path-parameter-fv-nv #:get-path-parameter-iv-nv
             #:cover-stroke-path-instanced-nv
             #:cover-fill-path-instanced-nv #:cover-stroke-path-nv
             #:cover-fill-path-nv #:path-fog-gen-nv #:path-tex-gen-nv
             #:path-color-gen-nv #:path-cover-depth-func-nv
             #:stencil-stroke-path-instanced-nv
             #:stencil-fill-path-instanced-nv
             #:stencil-stroke-path-nv #:stencil-fill-path-nv
             #:path-stencil-depth-offset-nv #:path-stencil-func-nv
             #:path-dash-array-nv #:path-parameter-f-nv
             #:path-parameter-fv-nv #:path-parameter-i-nv
             #:path-parameter-iv-nv #:transform-path-nv
             #:interpolate-paths-nv #:copy-path-nv #:weight-paths-nv
             #:path-glyph-range-nv #:path-glyphs-nv #:path-string-nv
             #:path-sub-coords-nv #:path-sub-commands-nv
             #:path-coords-nv #:path-commands-nv #:is-path-nv
             #:delete-paths-nv #:gen-paths-nv
             #:load-nv_path_rendering #:+set-amd+
             #:+replace-value-amd+ #:+stencil-op-value-amd+
             #:+stencil-back-op-value-amd+ #:stencil-op-value-amd
             #:load-amd_stencil_operation_extended
             #:is-image-handle-resident-nv
             #:is-texture-handle-resident-nv
             #:program-uniform-handleui-64v-nv
             #:program-uniform-handleui-64-nv
             #:uniform-handleui-64v-nv #:uniform-handleui-64-nv
             #:make-image-handle-non-resident-nv
             #:make-image-handle-resident-nv #:get-image-handle-nv
             #:make-texture-handle-non-resident-nv
             #:make-texture-handle-resident-nv
             #:get-texture-sampler-handle-nv #:get-texture-handle-nv
             #:load-nv_bindless_texture #:end-conditional-render-nvx
             #:begin-conditional-render-nvx
             #:load-nvx_conditional_render
             ...
             #:+min-sparse-level-amd+ #:+min-lod-warning-amd+
             #:+texture-storage-sparse-bit-amd+
             #:texture-storage-sparse-amd #:tex-storage-sparse-amd
             #:load-amd_sparse_texture
             #:+texture-memory-layout-intel+ #:+layout-default-intel+
             #:+layout-linear-intel+
             #:+layout-linear-cpu-cached-intel+
             #:map-texture-2d-intel #:unmap-texture-2d-intel
             #:sync-texture-intel #:load-intel_map_texture
             #:draw-texture-nv #:load-nv_draw_texture)) 
